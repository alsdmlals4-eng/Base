...
활성 프로젝트·분야 본책은 JSON이 책임 원본이다. DOCX와 PDF를 독립 원본으로 수동 수정하지 않는다. 활성 `*_기획서.md`, `DISCIPLINE_BIBLE.md`, `PROJECT_MASTER_PLAN.md`는 만들지 않는다.
...
- `DESIGN_DOCUMENT_REGISTRY.json`
- 프로젝트 종합·분야별 JSON 본책
- JSON에서 생성한 DOCX·PDF·다이어그램·승인 이미지 구조
- `SKILL_REGISTRY.json`과 사람용 스킬맵 DOCX/PDF
...
- 사람용 출력: `PROJECT_SKILL_MAP.docx/.pdf/.assets`

모든 의미 있는 호출은 Learning Log에 결과·실패·예외·사용자 피드백과 스킬 변경 필요성을 기록한다. 스킬 본문은 근거가 있을 때만 갱신한다.

### Active Context·Handoff

`skills/maintaining-project-context-and-handoff/SKILL.md`를 사용한다. 현재 상태, 다음 작업, 위험과 읽기 순서만 압축하고 JSON 본책의 전문을 복제하지 않는다.
...
2. 관련 JSON 본책과 Design Document Registry를 갱신한다.
3. 관련 스킬·Skill Registry·Learning Log를 갱신한다.
4. JSON 또는 승인 이미지가 바뀌면 DOCX·PDF·다이어그램·Manifest를 재생성한다.
