...
공용 구조와 상태·발행 정책의 단일 설명 원본은 `docs/OPERATING_MODEL.md`다. 이 문서는 요청을 해당 실행 Skill로 라우팅하는 역할만 가진다. L1 이상 실행 작업은 `PLAN → BUILD → REVIEW`를 모두 거쳐야 하며, PLAN 또는 BUILD만 끝난 상태를 완료로 보고하지 않는다. L0 오탈자·단순 설명·동일 검사 재실행처럼 실행 산출물이나 변경이 없는 요청만 제외한다.
