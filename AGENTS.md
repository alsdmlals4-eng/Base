...
Base는 여러 게임 프로젝트가 공유하는 **[학습형] [공용]** 규칙·Method·Skill·Template·Case·Test의 원본이다. 프로젝트 저장소는 Base를 그대로 복제하지 않고 자신의 세계관, 수치, 엔진, 실제 경로, 승인 자산과 구현 상태에 맞게 분화·적용·검증한다.
...
`전부 살펴본다`는 저장소 모든 파일과 스킬을 무작정 읽는 뜻이 아니다. `START_HERE.md`, `docs/DOCUMENTATION_MAP.md`와 `skills/SKILL_REGISTRY.json`으로 현재 작업에 필요한 공용 책임 원본과 최소 스킬 집합을 선별한다.
...
`docs/[기획서]`, `src/[기획서]` 같은 중첩 현행 복제본을 만들지 않는다. 운영 중인 기존 프로젝트의 경로는 감사와 사용자 승인 없이 강제로 이동하지 않는다.
...
- Active Context·Handoff 최신화: `skills/maintaining-project-context-and-handoff/SKILL.md`
...
- Documentation Map과 Skill Registry로 현재 작업에 필요한 현행 문서와 스킬만 선택한다.

## 11. 분야별·Foundation 스킬의 항상 학습

여러 분야에서 공통 사용하는 절차는 foundation에 한 번만 둔다. 각 분야는 실제 본책·경로·산출물·검증을 가진 독립 프로젝트 스킬로 분화한다.

각 스킬 필수 계약:

- skill ID·분야·상태
- 사용·비사용 조건과 trigger tags
...
모든 의미 있는 스킬 호출은 Learning Log에 다음을 기록한다.
...
스킬 본문을 매번 무조건 바꾸지 않는다. 반복 실패, 새 예외, 책임·경로·검증 변경처럼 근거가 있을 때 갱신한다. 한 번 성공한 방법을 즉시 공용 강제 규칙으로 만들지 않는다.
...
4. 호출한 프로젝트 스킬과 Learning Log를 실제 결과에 맞게 갱신한다.
5. 스킬 변경이 필요 없으면 그 이유를 기록한다.
6. 이미지·자산·PDF Manifest와 파생본 영향을 확인한다.
7. 실행한 검증, 미검증, 남은 위험을 분리한다.
8. 이동·통합·제거 파일의 참조와 보존 대조를 확인한다.
9. 프로젝트 고유 결과와 공용 재사용 원리를 분리한다.
10. 공용 후보는 기존 Base 원본과 중복을 확인해 Method·Skill·Template·Case·Test에 반영한다.
11. 새 AI가 10분 안에 방향·상태·다음 작업·최소 스킬·검증을 찾는지 확인한다.
12. Base 변경 후 프로젝트 `BASE_RULES_VERSION.md` 후속 동기화를 기록한다.

설치·마이그레이션·주요 게이트·콜드 스타트 실패 후에는 `skills/verifying-game-project-operating-system/SKILL.md`로 Health Review를 수행한다.
...
- 공용 Template·Research·Case·Test
- 공용 Skill Registry와 학습 기록
...
- 프로젝트 Skill Registry와 분야별 프로젝트 스킬
- 공용화 전 관찰·가설
...
- 주 책임·영향 분야
- 호출한 foundation·분야 스킬과 호출하지 않은 후속 스킬
- 변경 파일과 이유
- 유지한 기존 동작·결정·자산
- 실행한 검증과 결과
- 미검증·불일치·남은 위험
- 본책·게이트·Registry·스킬·Learning Log·Manifest·PDF 최신화
- 루트 `[기획서]` 위치와 중첩 복제 여부
- 보존·통합·백업·보류·제거 후보
- 콜드 스타트와 Health Review 결과
- 프로젝트 전용으로 유지한 내용
