...
- **`BENCHMARK_FIRST_MODULAR_UI_PRODUCTION`**: 신규 시스템·화면·UI/시각 설계는 `docs/knowledge/game-development/BENCHMARK_FIRST_MODULAR_UI_PRODUCTION.md`를 먼저 적용한다. 외부 게임·공개 구현 비교 → 구조 후보 → 프로젝트 정본/기존 구현/승인 자산 적합성 → 최소 준비 순서이며, 이 분야의 설계 조사에서는 아래 internal-first preflight 순서에 우선한다. 적용 권위·쓰기사전 fresh-read·고유 규칙·기존 재사용 의무는 보존한다. 이미지는 독립 부품과 조합을 기본으로 하고 실제 시각 적용에는 인게임 캡처를 남긴다.
