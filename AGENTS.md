...
사람 기본 열람 → 기획서 PDF
사람 문서 검토 → 기획서 DOCX
시각 자료 → 기획서.assets/generated + approved
최신성 → 기획서_PUBLICATION_MANIFEST.json
전체 라우팅 → DESIGN_DOCUMENT_REGISTRY.json
```

- 활성 프로젝트·분야 본책은 구조화 JSON이 책임 원본이다.
- DOCX·PDF·다이어그램은 생성되는 사람용 파생본이다.
- DOCX·PDF를 별도 원본으로 수동 유지하지 않는다.
- 활성 `*_기획서.md`, `DISCIPLINE_BIBLE.md`, `PROJECT_MASTER_PLAN.md`는 만들지 않는다.
- `START_HERE`, `ACTIVE_CONTEXT`, `DOCUMENTATION_MAP`, 작업 절차·Skill처럼 빠른 라우팅이 필요한 운영 문서는 Markdown을 유지할 수 있다.
...
- JSON 기획서·DOCX/PDF 발행: `publishing-discipline-bibles`
...
- 같은 내용을 JSON·Skill·운영 문서에 장문 복사하지 않는다.
...
JSON, 승인 이미지 또는 생성기가 바뀌면 DOCX·PDF·다이어그램·Manifest를 같은 작업에서 재생성한다.
...
- Asset ID·채택 요소·비채택 요소·실제 캡처를 JSON에 연결한다.
...
2. 관련 기획서 JSON·Design Document Registry가 최신이다.
3. 관련 Skill Registry·Learning Log가 최신이다.
4. JSON·이미지 변경 시 DOCX·PDF·다이어그램·Manifest가 재생성됐다.
