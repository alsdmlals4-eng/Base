...
- 작업 전 benchmark·역공학과 legacy/context hygiene: `managing-project-intake-and-work-contract` + `references/work-decomposition-and-sequencing.md`

## 2. L1+ 작업 진입·판단 불변식

### UI·시각 제작의 benchmark-first 보완 계약

`BENCHMARK_FIRST_MODULAR_UI_PRODUCTION` / `PROJECT_PIN_UNCHANGED`

게임 시스템의 화면 표현·메뉴·페이지·탭·대화창·시스템 창·UI 스킨 작업은 외부 게임의
행동·공개 구현 벤치마크로 후보 구조를 먼저 세운 뒤, 현재 프로젝트 정본·승인 자산·소비처에
대조한다. 실제 수정·자산 제작 전 fresh-read와 승인 경계는 유지한다. 이미지 부품을 독립
원본으로 제작하고 여러 화면에 조합·재사용하는 것을 기본으로 한다. 상세 절차는
`skills/auditing-and-refining-ui-art/references/ui-surface-production-readiness.md`를 적용한다.
이는 현재 프로젝트의 Base version lock·고유 규칙을 조용히 교체하는 권한이 아니며,
프로젝트 적용은 기존 owner에 exact source commit과 보완 범위를 명시해 채택한다.
`EXTERNAL_ORDER_OVERRIDE_FOR_UI`: 아래 일반 재사용 조사 순서보다 이 범위의 외부 벤치마크 순서가 우선한다.
재사용 대조 자체는 제작 전 필수이며 권한·승인·fresh-read·보호 경계를 생략하지 않는다.
