...
- 승인 근거: 2026-08-13 현재 ChatGPT 세션 사용자 지시 — “할루시네이션 현상방지, 우리가 의도한대로 제대로 구현이 되었는지 확인하는 스킬을 Base에 추가하고 작업구조에도 반영” 및 병합까지 수행 요청
- 구현 PR: 구현 브랜치에서 생성 후 이 제안의 Registry 항목에 연결
...
이 공백은 완전히 허위인 주장뿐 아니라 다음과 같은 더 흔한 실패를 허용한다.
...
| 기존 Skill에 Mode·reference·Template·행동 평가 흡수 | 기존 권한·입출력·판정과 일치, active Skill 수 불변 | 본문이 과밀해질 수 있음 | 채택; 상세 절차는 reference로 분리 |
...
SimpleQA는 긴 응답을 짧고 원자적으로 검증 가능한 주장으로 분해하면 사실성 평가가 더 다루기 쉬워진다는 문제의식을 사용하며, 정답·오답·미응답을 구분한다. Base는 이를 결과 문장 전체의 모호한 신뢰 점수 대신 중요한 원자 주장별 `VERIFIED / CONTRADICTED / UNVERIFIED` 판정으로 변형한다.

- https://openai.com/index/introducing-simpleqa/

### 현업 Eval 도구의 공통 패턴

Arize Phoenix, LangSmith, Braintrust, Promptfoo의 공식 문서는 대체로 다음 조합을 사용한다.

- 고정된 dataset·reference 또는 experiment snapshot
- 가능한 사실은 deterministic evaluator로 검사
- 의미·품질 판단은 rubric·judge를 보조적으로 사용
- CI 회귀와 production feedback을 분리
- hallucination, faithfulness, fabricated citation, tool selection을 독립 평가
...
### 4.2 `INTENT_IMPLEMENTATION_FIDELITY_MATRIX`
...
- L1 이상 AI/Agent 완료 보고, 외부 사실, 구현 충실도, 테스트·병합 주장은 이 Gate를 사용한다.
- L2 이상 복합 변경은 기존 `FEATURE_SPEC_TRACEABILITY_PACKET`의 ID를 재사용한다.
- 원자 주장 원장은 제품 정본을 복제하지 않고 검증 시점의 Evidence record로 남긴다.
...
판정: `APPROVED_FOR_IMPLEMENTATION`. P0/P1 미해결 충돌 없음. 외부 SaaS 도입과 자동 진실 점수는 범위에서 제외한다.

## 7. 검증 계약

### RED

전용 계약 테스트가 새 reference·Mode·Template·행동 평가를 요구하도록 먼저 추가한다. 생산 계약이 없는 exact HEAD에서 GitHub Actions 실패를 관찰한다.

### GREEN

최소 owner/reference/workflow 변경을 적용한 exact HEAD에서 다음을 확인한다.
...
- Copilot 또는 가능한 독립 reviewer finding 검토
...
문제가 생기면 구현 PR의 merge commit을 revert한다. 새 reference와 Mode 연결, Template 섹션, fixture·test를 함께 되돌려 부분 활성 상태를 남기지 않는다. BCP 제안 기록은 역사로 유지하고 Registry 상태를 `REJECTED` 또는 후속 판정으로 갱신한다.

## 9. 승인과 구현

- 사용자 승인: 2026-08-13 현재 ChatGPT 세션의 직접 요청과 병합 지시
- 제안 상태: `APPROVED_FOR_IMPLEMENTATION`
- 구현 권한: 이 문서의 영향 경로·보호 대상·검증 계약 범위
- 구현 PR: 생성 후 Registry `implementation_pr`에 기록
