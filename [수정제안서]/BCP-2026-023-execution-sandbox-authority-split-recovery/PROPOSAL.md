# BCP-2026-023-execution-sandbox-authority-split-recovery — Execution Sandbox Remote-Authority Split Recovery
...
동시성 기록: 최초 GRIMOIRE proposal #293은 `BCP-2026-022-*`로 exact-head validation까지 통과했으나 final race check에서 다른 프로젝트 PR #294가 같은 numeric slot 022를 사용한 사실을 확인했다. #293은 미병합 종료했고 다른 프로젝트를 수정하지 않은 채 이 제안만 current Base main에서 023으로 재할당했다.
...
3. read-only 대안으로 시도한 `git ls-remote origin refs/heads/main`도 `github.com:443` 연결 실패로 종료되었다.
...
remote repository freshness와 local executor readiness를 **한 executor의 동일 Git/network capability 성공 여부에 묶은 것**이 원인이다. 실행 채널은 working-tree read, linked-worktree administrative metadata write, outbound network, credentials, control-plane connector 권한을 서로 다르게 가질 수 있다.

공식 Git `git-worktree` 문서는 linked worktree가 repository `$GIT_DIR/worktrees/<id>` 아래 private administrative directory를 사용하고 `$GIT_COMMON_DIR`로 common repository state를 참조한다고 설명한다. 따라서 working tree를 읽을 수 있다는 사실이 administrative path write capability까지 증명하지 않는다.

공식 GitHub required-status-check 문서는 현재 validation commit identity가 중요하며 head와 test-merge/merge-queue identity를 구분해야 한다고 설명한다. 이 제안의 receipt split 명칭 자체는 Git/GitHub 규정이 아니라, **exact identity와 freshness를 유지하면서 다른 trusted capability channel을 결합해야 했던 프로젝트 증거에서 도출한 inference**다.
...
- current PR validation identity: 현재 required check producer/commit identity 분리.

그러나 current Base에서 **local executor가 remote network/shared Git metadata capability를 잃었을 때 별도 trusted connector가 remote repository freshness를 공급하고, local exact state/test receipt는 독립적으로 유지하는 fallback contract**는 확인되지 않았다.
...
5. 같은 capability failure를 의례적으로 반복해 independent local verification을 계속 막지 않는다.
...
다음은 GRIMOIRE 전용이며 Base 공용 규칙으로 승격하지 않는다.

- Task8 local worktree 경로, branch/head, 9-path allowlist.
- HiGodot/Hera/GUT/Godot exact versions, ports, project-scoped `CODEX_HOME`와 Hera profile.
...
- required remote write를 수행하고 final race check할 approved capable channel이 없다.
...
6. shared proposal namespace는 branch creation receipt가 있어도 final race check가 필요하다. 실제로 이 제안은 022 collision을 발견해 own proposal만 023으로 재할당했다.
...
1. 단일 executor가 모든 capability를 가진 정상 경로 — 추가 ceremony 없이 기존 route 유지.
