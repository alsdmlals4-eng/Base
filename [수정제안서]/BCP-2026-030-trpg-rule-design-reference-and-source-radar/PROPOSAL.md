...
세부 Source Record와 관찰은 `evidence/TRPG_SOURCE_SCAN_2026-08-24.md`와 `evidence/TRPG_SOURCE_RIGHTS_AND_FRONTIER_ADDENDUM_2026-08-24.md`에 둔다.
...
3. **Character truth와 mechanical effect 분리**: Fate의 aspect/stunt 계열처럼 '무엇이 사실인가'와 '그 사실이 어떤 규칙 효과를 내는가'를 분리하면 자유도와 판정 명료성을 동시에 얻을 수 있다.
...
9. **SRD와 제품의 권리 경계**: Fate/Blades/24XX/Dungeon World처럼 공개 라이선스가 명시된 SRD와, TRPG Club의 상용 작품/부속 자료처럼 관찰만 가능한 자료를 같은 재사용 권한으로 취급하면 안 된다. Year Zero Engine·BRP처럼 전용 라이선스/ORC가 있는 경우도 해당 계약 범위만 사용해야 한다.
10. **VTT/ORPG는 룰과 별도 UX 층**: `adventurekeeper`의 Naver 글은 직접 접근이 차단됐으므로 구체 내용을 확정하지 않는다. 이 계열 자료는 판정 규칙보다 장면 전환·캐릭터 표현·BGM·로그·자동화 수준 등 온라인 세션 운영 UX 참고로 분리한다.
11. **Reference schema와 teaching order는 다르다**: GUMSHOE SRD처럼 디자이너용 reference는 완성된 룰북의 교육 순서를 대신하지 않는다. Dungeon World·인세인·Mothership·Ironsworn 등은 예시/리플레이/퀵스타트/flow를 별도 teaching surface로 둔다.
...
1. `docs/knowledge/game-development/TRPG_RULE_DESIGN_AND_PLAY_REFERENCE.md`
   ...
   - 룰북 설계에는 `PLAYER_PROMISE → 30초/5분 FLOW → PLAY EXAMPLE → CHARACTER CREATION → CORE RESOLUTION → SIGNATURE LOOP → GM PROCEDURE → QUICKSTART → REFERENCE/DATA → OPTIONAL MODULES`를 **기본 후보**로 두되 장르별 예외를 허용한다.
2. `docs/knowledge/game-development/TRPG_REFERENCE_SOURCE_RADAR.md`
   ...
   - 각 룰북/source에는 단순 링크가 아니라 `signature_experience`, `problem_solved`, `mechanic_solution`, `teaching_order`, `progressive_disclosure`, `support_artifacts`, `gm_player_information_boundary`, `adopt_adapt_reject`, `limitations`를 함께 기록한다.
   - exact source access가 불가능하면 `UNVERIFIED_DIRECT`로 남기고 검색 스니펫이나 기억으로 내용을 확정하지 않는다.

재사용 가능한 설계 계약 후보:

- `FICTION_FIRST_TRIGGER_CONTRACT`: fiction-triggered system에서 허구의 선언이 규칙 발동 조건을 충족할 때 판정하고 결과를 다시 허구에 반영한다. 모든 TRPG에 move trigger를 강제하지 않는다.
...
- `RISK_AND_EFFECT_ARE_DISTINCT`: 위험의 크기와 목표에 미치는 효과의 크기를 구분하는 편이 플레이 선택을 개선하는 경우 별도 축으로 판단한다.
...
- `REFERENCE_SCHEMA_IS_NOT_TEACHING_ORDER`: designer SRD/data schema 순서를 신규 플레이어용 룰북 설명 순서로 그대로 복제하지 않는다.
...
- 룰북/퀵스타트/시트가 같은 규칙을 서로 다른 순서와 밀도로 가르쳐야 할 때.
...
- 단순히 목차 순서가 유명한 룰북과 비슷하다는 이유만으로 교육 품질이 높다고 판단하지 않는다. first-use comprehension과 table reference 비용을 별도로 본다.
...
5. **2d6/PbtA 과적합**: Dungeon World 같은 2d6 결과 구간은 한 사례군이다. d20, percentile, dice pool, step-die, diceless/token 등 다른 해법과 비교 후 프로젝트별 선택한다.
6. **Fate 과적합**: 면모/서사점은 서사 중심 플레이에 강하지만 tactical resource planning이나 mystery certainty를 그대로 대체하지 않는다.
7. **직접 접근 실패**: Dropbox exact folder와 Naver `adventurekeeper`는 현재 web 도구로 직접 읽히지 않았다. 각각 publisher 연결/주변 색인으로 존재 목적만 확인했으며 세부 내용은 확정하지 않는다.
8. **설명 순서 과적합**: `플레이 흐름 먼저`가 대체로 유리해도 감정 중심 호러는 리플레이-first, setting permission이 중요한 자유 설정형은 setting consensus-first, 강한 클래스 판타지는 character-first가 더 나을 수 있다.
...
2. Source Radar의 각 source에 `source_tier`, `access_status`, `rights_status`, `signature_experience`, `problem_solved`, `mechanic_solution`, `teaching_order`, `support_artifacts`, `use_for`, `limitations`, `recheck_condition`이 있는지 확인.
3. 공식/open source와 상용/reference-only source가 구분되는지 확인.
4. 기존 benchmark/reverse-engineering owner와 중복되는 새 workflow를 만들지 않았는지 확인.
5. 룰북 목차/데이터 구조와 실제 first-session teaching order가 분리되어 기록되는지 확인.
6. 최소 5회 full-scope adversarial review 후 blocking finding 0일 때만 구현 merge.
...
- 사용자 승인 근거: 2026-08-24 현재 작업 대화의 `base에 trpg 자료로 추가하고. 더 많은 링크를 찾아봐`, 뒤이은 5개 링크 재확인, 그리고 `룰북 각각 특징과 어떻게 풀어냈는지, 설명은 어떤 순서로 어떻게 진행했는지도 생각해서 정리` 지시.
- 현재 lifecycle 해석: **Base 추가 방향과 분석 축은 사용자 승인됨.** 다만 Base 규칙에 따라 제안 PR → 승인 상태 PR → 구현 PR의 경계를 분리한다.
