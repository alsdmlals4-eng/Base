...
- Proposal ID: `BCP-2026-016-serial-fiction-reconciliation-frontier-and-derived-continuity-guard`
...
BCP-012 `serial-fiction-canon-migration-debt`는 새 Canon의 즉시 권위와 기존 DRAFT migration debt를 분리하는 공용 lifecycle을 이미 소유한다. BCP-016은 이를 대체하지 않는다. 이번 프로젝트에서 추가로 드러난 것은 다음 edge다.
...
특히 외부 최신 원고와 legacy 저장 원고가 같은 chapter 번호를 공유해도 사건 배치가 다를 수 있으므로, 검증되지 않은 경계를 index·reverse outline·scene graph 같은 파생 consumer가 정상 `previous/next` 관계로 자동 연결하면 사건 되감기나 거짓 continuity가 생성될 수 있다.

BCP-016은 새 broad Skill을 만들자는 제안이 아니다. 승인 후 구현이 필요하다면 기존 `developing-and-revising-serial-fiction: canon-and-continuity`와 `auditing-canonical-reference-freshness` owner에 흡수하는 것을 우선한다.

## 관찰과 증거

### 1. BCP-012 이후 실제 migration에서 frontier가 필요했다

Coc-Fiction PR #14는 새 Canon과 기존 대량 DRAFT 충돌을 즉시 전수 치환하지 않고 bounded legacy reconciliation debt로 분리했다. 이 단계에서는 “어느 artifact가 debt인지”를 안전하게 고정할 수 있었다.

그 뒤 외부 최신 제1~105화와 GitHub legacy 225화 저장 원고를 실제로 비교하기 시작하면서, debt 목록만으로는 다음 사실을 표현하기 부족했다.
...
새 제5화는 legacy 제6화보다 사건이 더 진행된 상태였다. 따라서 제5→legacy 제6화를 정상 연속으로 두면 이미 지나간 사건이 다시 시작되는 회귀가 생겼다. 프로젝트는 경계를 억지로 연결하지 않고 `next/previous = null`에 해당하는 migration-boundary 상태로 차단했다.
...
여기서 핵심은 `5→6` 경계가 정상 연결 가능한 검증 영역으로 편입되고, 미검증 경계가 `10→11`로 이동한다는 점이다.

따라서 migration debt는 단순 정적 backlog만이 아니라 다음과 같은 **reconciliation frontier**로도 표현될 수 있다.
...
PR #17의 reverse-outline/index 작업에서 migration boundary가 없으면 current chapter 뒤에 legacy chapter가 정상 next로 생성될 수 있었다.

프로젝트에서 사용한 실제 JSON 파일명과 composer 구조는 Coc-Fiction 전용이지만 공용 원리는 다음과 같다.
...
PR #17 디버깅에서 한 chapter를 일반 override와 migration-boundary override가 동시에 소유해 composed reverse-outline가 실패했다.

잘못된 대응은 duplicate guard를 느슨하게 만드는 것이다. 프로젝트는 guard를 유지하고 한 current owner만 남도록 overlap을 제거했다.
...
이 항목은 현재 한 프로젝트에서 강하게 확인된 evidence이므로 독립 공용 규칙으로 즉시 승격하지 않고 BCP-016 구현 검토용 반례로 보존한다.

### 5. `최종`이라고 적힌 외부 artifact도 자동 Canon authority가 아니다

사용자가 제공한 최신 외부 원고는 기존 GitHub manuscript보다 새로운 편성과 문장을 포함했지만, 파일명이 `최종`이라는 이유로 225화 저장 원고에 blind overwrite하지 않았다.

bounded bundle마다 다음 순서를 사용했다.
...
이 원리는 기존 serial-fiction source/Canon priority와 정합적이며 새 Skill을 요구하지 않는다.
...
BCP-016은 다음 fail-closed invariant를 제안한다.

```text
frontier advancement may be recorded as candidate state,
but verified-prefix / reconciliation-complete claims require the declared validation gate to be GREEN.
```

실패한 run의 원인을 이 proposal이 임의로 단정하지 않는다. 현재 확인된 사실은 completed scene-pass validator가 실패했다는 것뿐이다. Coc-Fiction 프로젝트의 해당 failure 수정은 Base proposal 병합과 별도 프로젝트 작업이다.
...
Coc-Fiction PR #15→#16에서 live `ACTIVE_CONTEXT/HANDOFF`가 자기 자신을 운반하는 merge 뒤 즉시 stale해지는 현상도 재현됐다.

이 문제는 이미 BCP-013 `post-merge-continuation-state-reconciliation`이 소유하므로 BCP-016에서 중복 제안하지 않는다. supporting evidence로만 연결한다.

### BCP-009와의 관계

BCP-009 `serial-fiction-writing-and-revision-discipline`은 집필·퇴고 품질, source/Canon, POV, continuity discipline의 광역 owner다. 그러나 대량 legacy 저장 원고의 **부분 migration topology와 derived consumer boundary**는 BCP-012 이후 새로 드러난 운영 edge이므로 BCP-016에서 proposal evidence로 분리한다.
...
장편·연재 artifact를 단계적으로 새 Canon/편성에 맞출 때 최소 의미 상태를 다음처럼 분리한다.
...
```yaml
reconciliation:
  verified_prefix_end: 10
  migration_boundary_after: 10
  legacy_tail_starts_at: 11
  whole_artifact_continuity: NOT_YET_CLAIMED
  verification: PASS | FAIL | IN_PROGRESS | UNVERIFIED
  next_scope: 011-015
```
...
- `MANUSCRIPT_INDEX_OVERRIDE_*`, `REVERSE_OUTLINE_OVERRIDE_*`, `SCENE_PASS_REGISTRY.json` 같은 프로젝트 경로
- 225화라는 legacy storage 숫자 자체
...
POV·후크·캐릭터 QA는 Coc-Fiction에서 유용했지만, 고정 숫자보다 “새 POV가 실제 정보·감정·외부평가를 추가하는가”라는 기존 Base POV 가치 필터로 충분하므로 이 BCP의 공용 승격 범위에서 제외한다.

## 적용 조건과 비사용 조건

### Use When

- 장편/연재 artifact가 이미 많이 존재하고 최신 Canon 또는 편성으로 단계적 migration 중이다.
...
- 단지 프로젝트의 work-unit 크기를 Base에 강제하고 싶을 뿐인 경우.
...
PR #19처럼 여러 검사 중 하나가 실패하면 declared merge/promotion gate 기준으로 전체 verified claim을 중단한다. 어떤 검사가 blocking인지는 각 프로젝트 계약이 정한다.
...
- AWS ADR guidance는 새 Decision이 이전 것을 supersede해도 legacy non-compliant code/artifact가 자동 해결되지 않으며 점진 업데이트 또는 debt task로 별도 관리해야 한다고 설명한다. 이는 BCP-012의 decision/migration 분리와 정합적이다.
- Martin Fowler의 Parallel Change는 backward-incompatible 변경을 `expand → migrate → contract`로 나눠 소비자를 점진적으로 옮긴다. 이는 verified prefix가 증가하고 legacy tail이 줄어드는 frontier 사고와 유사하지만, Base는 software interface의 dual-write schema를 fiction에 그대로 강제하지 않는다.
- GitHub Docs의 three-dot comparison은 merge base 이후 topic branch가 도입한 delta를 중심으로 보여준다. 이는 stale 전체 history를 통째로 부활시키기보다 current base 위 unique delta를 재검토하는 기존 Base 운영과 정합적이다.
...
- `[수정제안서]/BCP-2026-016-serial-fiction-reconciliation-frontier-and-derived-continuity-guard/PROPOSAL.md`
- `[수정제안서]/BCP-2026-016-serial-fiction-reconciliation-frontier-and-derived-continuity-guard/evidence/COC_FICTION_RECONCILIATION_FRONTIER_EVIDENCE.md`
...
Given two active migration consumers claim current authority over the same chapter/artifact,
...
- current BCP-012/013을 supersede하지 않음
- existing archive/history는 rewrite 강제 없음
- 특정 fiction data schema 도입 강제 없음
- 기존 project work-unit 크기 변경 없음

### Security / cost / operational risk

보안상 새로운 secret·권한·runtime write를 요구하지 않는다.

비용은 추가 상태와 검증 discipline에서 발생한다. 너무 작은 migration까지 frontier로 모델링하면 운영 비용이 커질 수 있으므로 Do Not Use 조건을 유지한다.

## 롤백

이번 proposal-only PR은 `[수정제안서]/**` 세 파일만 변경한다. 문제가 있으면 해당 proposal/Registry delta만 revert할 수 있으며 Base active behavior와 프로젝트 원고에는 영향이 없다.
