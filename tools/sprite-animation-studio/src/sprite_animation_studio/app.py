...
def _normalize_delivery_payload(
    ...
    *,
    ...
    if (
        ...
        or delivery.get("tool_route_id") != tool_route_id
        or delivery.get("target_node_name") != target_node_name
...
def create_app(
    ...
    confirmed_deliveries: dict[str, tuple[str, str, str, dict[str, object]]] = {}
    ...
    def confirm_delivery(run_id: str) -> dict[str, object]:
        try:
            ...
            target_name = _ROUTE_TARGETS[route_id]
            ...
            normalized = _normalize_delivery_payload(
                delivery,
                project_id=str(record.request.project_id),
                run_id=run_id,
            ...
            with confirmed_delivery_lock:
                confirmed_deliveries[run_id] = (expected_sha256, route_id, target_name, normalized)
            ...
            raise HTTPException(status_code=404, detail="run not found") from error
        except (RunBlockedError, DeliveryBlockedError, HubDeliveryError, KeyError, ValueError) as error:
            raise HTTPException(status_code=409, detail=str(error)) from error
    ...
    def delivery_status(run_id: str) -> dict[str, object]:
        try:
            with confirmed_delivery_lock:
                cached = confirmed_deliveries.get(run_id)
            if cached is None:
                raise RunBlockedError("confirmed delivery is required before status refresh")
            expected_sha256, route_id, target_name, previous = cached
            record = service.get_run(run_id)
            if record.status != "exported" or record.export is None:
                raise RunBlockedError("confirmed exported run is unavailable")
            if service.delivery_route_id(run_id) != route_id:
                raise RunBlockedError("confirmed delivery route changed after confirmation")
            _read_staged_file(project_root, record.export.atlas, expected_sha256=expected_sha256)
            if sender is None:
                raise HubDeliveryError("Tool Hub confirmed delivery is unavailable")
            delivery = sender.status(str(previous["delivery_id"]))
            normalized = _normalize_delivery_payload(
                delivery,
                project_id=str(record.request.project_id),
                run_id=run_id,
                ...
                tool_route_id=route_id,
                target_node_name=target_name,
                expected_delivery_id=str(previous["delivery_id"]),
            )
            with confirmed_delivery_lock:
                confirmed_deliveries[run_id] = (expected_sha256, route_id, target_name, normalized)
            return _confirmation_response(
    ...
    def confirmed_download(run_id: str) -> Response:
        try:
            with confirmed_delivery_lock:
                cached = confirmed_deliveries.get(run_id)
            if cached is None:
                raise RunBlockedError("confirmed delivery is required before download")
            expected_sha256, route_id, _target_name, _previous = cached
            record = service.get_run(run_id)
            if record.status != "exported" or record.export is None:
                raise RunBlockedError("confirmed exported run is unavailable")
            if service.delivery_route_id(run_id) != route_id:
            ...
            return Response(
                ...
                headers={
                    "Content-Disposition": f'attachment; filename="{record.export.atlas.name}"',
