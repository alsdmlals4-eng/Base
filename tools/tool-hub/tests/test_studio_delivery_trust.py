...
def test_sprite_internal_delivery_accepts_only_authenticated_dedicated_routes(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    client = _registered_hub(tmp_path)
    ...
    missing = client.post(
        "/internal/studio-delivery/sprite-missing",
        content=payload,
        headers=_studio_headers(),
    )
    ...
    character = client.post(
        "/internal/studio-delivery/sprite-character",
        content=payload,
    ...
    unknown = client.post(
        "/internal/studio-delivery/sprite-unknown",
        content=payload,
        headers=_studio_headers("unknown_route"),
    )
    assert unknown.status_code == 409
    assert unknown.json()["detail"] == "DELIVERY_TOOL_ROUTE_UNAVAILABLE"

    sprite = client.post(
        "/internal/studio-delivery/sprite-action",
        content=payload,
        headers=_studio_headers("sprite_action_runs"),
    )
    effect = client.post(
        "/internal/studio-delivery/effect-stages",
        content=payload,
