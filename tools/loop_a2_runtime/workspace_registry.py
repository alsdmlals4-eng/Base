...
class WorkspaceOwnershipRegistry:
    ...
    def _assert_safe_tree(self, path: Path, *, allow_missing: bool) -> None:
        ...
        if candidate != runtime and runtime not in candidate.parents:
            raise WorkspaceOwnershipError("ownership path escapes runtime_root")
        try:
            relative = path.relative_to(runtime)
        except ValueError as exc:
            raise WorkspaceOwnershipError("ownership path is not lexically runtime-bound") from exc
        current = runtime
        ...
        for part in relative.parts:
            ...
            if current.is_symlink():
                raise WorkspaceOwnershipError("ownership path traverses a symlink")
            if not allow_missing and not current.exists():
                raise WorkspaceOwnershipError("ownership path is missing")
    ...
    def _payload(
        ...
        *,
        ...
        canonical_workspace = Path(workspace).resolve(strict=False)
        expected_workspace = (self.runtime_root / project / run).resolve(strict=False)
