...
def _bounded_text(value: str, *, maximum: int) -> bool:
    return bool(value) and len(value) <= maximum and "\x00" not in value
...
def build_subscription_handoff_packet(
    *,
    ...
    if workflow not in _WORKFLOW_TOOL:
    ...
    if isinstance(expected_png_count, bool) or not 1 <= expected_png_count <= 8:
        raise SubscriptionHandoffError("expected PNG count must be between 1 and 8")
    if (
        isinstance(min_dimension, bool)
    ...
    if any(not _bounded_text(item, maximum=240) or _contains_secret(item) for item in checklist):
