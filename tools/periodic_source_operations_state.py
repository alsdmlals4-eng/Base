...
def validate_actual_source_review_receipt(
    ...
    _, known_source_ids = _known_ledger_source_ids(ledger)

    _parse_iso_date(checked.get("scan_date"), "scan_date")
    ...
    checked["merged_base_contribution_refs"] = _string_list(
        checked.get("merged_base_contribution_refs"), "merged_base_contribution_refs"
    )
...
def reconcile_operations_ledger_from_receipts(
    ...
    Ledger through the normal reviewed weekly batch.
    """
    ...
    for source_id, events in material_events.items():
        row = by_id[source_id]
        existing_count = row.get(MATERIAL_COUNT_FIELD, 0)
        if (
        ...
        if existing_date_value is not None:
            existing_date = _parse_iso_date(existing_date_value, MATERIAL_DATE_FIELD)
        increment = sum(
