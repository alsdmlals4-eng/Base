...
The checker supports static scans and optional git-diff-aware coupled-change rules.
    ...
    for index, rule in enumerate(rules):
        ...
        for consumer in consumers:
            ...
            if tokens and not any(token in text for token in tokens):
                errors.append(
                    f"{label}: consumer does not reference current canonical source: {consumer}"
                )
    return errors
...
def check_coupled_changes(changed: set[str], rules: list[dict]) -> list[str]:
    ...
    for index, rule in enumerate(rules):
        ...
        if missing_all:
            errors.append(
                f"{label}: source change {triggered} requires changed companions {missing_all}"
            )
        if require_any and not any(
            ...
            errors.append(
                f"{label}: source change {triggered} requires at least one changed companion "
                f"from {require_any}"
            )
    return errors
...
def main() -> int:
    ...
    scan_globs = [str(item) for item in config.get("scan_globs", [])]
    ignore_globs = [str(item) for item in config.get("ignore_globs", [])]
    files = iter_text_files(root, scan_globs, ignore_globs)
    ...
    allowed_legacy = [str(item) for item in config.get("allowed_legacy_globs", [])]

    errors: list[str] = []
    errors.extend(check_legacy_references(root, files, aliases, allowed_legacy))
