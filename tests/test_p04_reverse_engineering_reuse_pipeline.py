...
class P04ReverseEngineeringReusePipelineTests(unittest.TestCase):
    ...
    def test_current_active_projects_have_reusable_module_catalog(self) -> None:
        registry = read("docs/knowledge/reuse/REUSABLE_MODULE_REGISTRY.md")
        ...
        self.assertIn("MODULE_CONTRACT_DEFINED", registry)
        self.assertIn("IMPLEMENTATION_NOT_BUILT", registry)
        self.assertIn("RIGHTS_REVIEW_REQUIRED", registry)
        self.assertIn("NOTION_HUMAN_VIEW", registry)

    def test_reusable_module_catalog_keeps_family_contracts_separate(self) -> None:
        gameplay = read("docs/knowledge/reuse/GAMEPLAY_AND_CONTENT_MODULES.md")
        production = read("docs/knowledge/reuse/PRODUCTION_TOOL_WORKFLOW_MODULES.md")
        visual = read("docs/knowledge/reuse/VISUAL_ASSET_MATERIAL_MODULES.md")
