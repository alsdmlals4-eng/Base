...
class LoopCompletionGateTests(unittest.TestCase):
    ...
    def _verified_receipt(self, capsule_path: Path) -> dict[str, object]:
        receipt = load_json(capsule_path.parent / "VERIFICATION_RECEIPT.json")
        receipt["status"] = "VERIFIED"
        receipt["exact_head_sha"] = "1" * 40
        receipt["checks"] = [{
            "check_id": "CORE_TESTS",
            "required": True,
            "status": "PASS",
    ...
    def test_required_not_run_and_stale_destination_block_completion(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            receipt = self._verified_receipt(capsule_path)
            receipt["checks"] = [{
                "check_id": "CORE_TESTS",
                "required": True,
                "status": "NOT_RUN",
    ...
    def test_placeholder_head_or_optional_only_evidence_cannot_close_completion(self) -> None:
        from tools.loop_contracts.bundle_validation import validate_completion
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            receipt = self._verified_receipt(capsule_path)
            receipt["exact_head_sha"] = "0" * 40
            write_json(receipt_path, receipt)
            self.assertIn("COMPLETION_HEAD_PLACEHOLDER", {item.code for item in validate_completion(capsule_path)})

            receipt = self._verified_receipt(capsule_path)
            receipt["checks"][0]["required"] = False
            receipt["destinations"][0]["required"] = False
            write_json(receipt_path, receipt)
            codes = {item.code for item in validate_completion(capsule_path)}
            self.assertIn("REQUIRED_CHECK_MISSING", codes)
            self.assertIn("REQUIRED_DESTINATION_MISSING", codes)
