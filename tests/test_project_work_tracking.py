...
def run_cli(value, *args):
    ...
    with tempfile.TemporaryDirectory() as directory:
        ...
        return subprocess.run([sys.executable, str(CLI), "--receipt", str(path), *args], text=True, capture_output=True, check=False)
...
def tracked_receipt():
    ...
    value["project_work_kanban"] = {
        ...
        "source_main_sha": SOURCE, "work_item_refs": ["PM-01"], "active_work_item_ref": "PM-01",
...
class ProjectWorkTrackingCLITests(unittest.TestCase):
    ...
    def test_default_start_gate_rejects_missing_pm_tracking(self):
        self.assert_rejected(receipt(), "project_work_kanban")

    def test_recorded_benchmark_blocker_does_not_authorize_execution(self):
        value = receipt()
        value["benchmark_preflight_receipt"] = {"state": "BLOCKED_UNVERIFIED", "blocked_sources": ["required original source"]}
        self.assert_rejected(value, "BLOCKED_UNVERIFIED")
    ...
    def test_verified_completion_is_counted(self):
        ...
        self.assertEqual(0, result.returncode, result.stdout + result.stderr)
        self.assertIn("1 / 1", result.stdout); self.assertIn("[x]", result.stdout)
    ...
    def test_missing_evidence_does_not_count_as_pass(self):
        value = done_receipt(); value["project_work_kanban"]["work_items"][0]["checklist"][0]["evidence"] = []
        self.assert_rejected(value, "evidence", "--phase", "closeout")

    def test_runtime_not_run_cannot_be_relabelled_done(self):
        value = done_receipt(); task = value["project_work_kanban"]["work_items"][0]
    ...
    def test_checkpoint_does_not_authorize_completion(self):
        value = tracked_receipt(); board = value["project_work_kanban"]
        board["work_items"][0].update(status="DONE", checkpoint_commit_sha="a" * 40)
    ...
    def test_required_acceptance_cannot_be_omitted(self):
        value = done_receipt(); value["project_work_kanban"]["work_items"][0]["acceptance_criteria"].append("AC-02")
        self.assert_rejected(value, "AC-02", "--phase", "closeout")

    def test_empty_work_items_are_not_a_work_plan(self):
        value = tracked_receipt(); value["project_work_kanban"].update(work_items=[], work_item_refs=[])
        self.assert_rejected(value, "work_items")

    def test_duplicate_ids_are_rejected(self):
        value = tracked_receipt(); board = value["project_work_kanban"]
        board["work_items"].append(copy.deepcopy(board["work_items"][0]))
        self.assert_rejected(value, "duplicate")

    def test_missing_required_work_ref_is_rejected(self):
        value = tracked_receipt(); value["project_work_kanban"]["work_item_refs"].append("PM-02")
        self.assert_rejected(value, "work_item_refs")

    def test_blocked_task_requires_reason_and_resume_condition(self):
        value = tracked_receipt(); value["project_work_kanban"]["work_items"][0]["status"] = "BLOCKED_UNVERIFIED"
        self.assert_rejected(value, "resume_condition")

    def test_unfinished_dependency_does_not_unlock_ready_task(self):
        value = tracked_receipt(); board = value["project_work_kanban"]
        task = copy.deepcopy(board["work_items"][0]); task.update(work_item_id="PM-02", status="READY", depends_on=["PM-01"])
        board["work_item_refs"].append("PM-02"); board["work_items"].append(task)
    ...
    def test_unhashable_input_reports_errors_instead_of_crashing(self):
        value = tracked_receipt(); value["work_level"] = []
    ...
    def test_done_requires_exact_verified_head_not_checkpoint_only(self):
        value = done_receipt(); value["project_work_kanban"]["work_items"][0].pop("verified_head_sha")
        self.assert_rejected(value, "verified_head_sha", "--phase", "closeout")
    ...
    def test_all_nested_malformed_fields_fail_without_traceback(self):
        ...
        paths = [("work_level",), ("benchmark_preflight_receipt", "state"), ("benchmark_preflight_receipt", "entries", 0, "disposition"), ("context_configuration_hygiene", "inventory", 0, "classification"), ("project_work_kanban", "work_item_refs"), ("project_work_kanban", "active_work_item_ref")]
        paths += [("project_work_kanban", "work_items", 0, field) for field in ("status", "depends_on", "acceptance_criteria", "required_evidence")]
        paths += [("project_work_kanban", "work_items", 0, field, 0, "status") for field in ("checklist", "verification")]
        for path in paths:
            for malformed in (None, [], {}, ["x", {}], True, 7):
                if path[-1] == "depends_on" and malformed == []: continue
                with self.subTest(path=path, malformed=malformed):
                    value = copy.deepcopy(tracked_receipt()); cursor = value
    ...
    def test_independent_blocker_does_not_stop_active_task(self):
        value = tracked_receipt(); board = value["project_work_kanban"]
        task = copy.deepcopy(board["work_items"][0]); task.update(work_item_id="PM-02", status="BLOCKED_UNVERIFIED", blocker="required runtime unavailable", resume_condition="engine becomes available")
        board["work_items"].append(task); board["work_item_refs"].append("PM-02")
        result = run_cli(value, "--phase", "resume", "--render-markdown")
        self.assertEqual(0, result.returncode, result.stdout + result.stderr)
        self.assertIn("0 / 2", result.stdout); self.assertIn("required runtime unavailable", result.stdout)
    ...
    def test_cycle_and_wip_violation_are_both_rejected(self):
        ...
        board["work_items"].append(task); board["work_item_refs"].append("PM-02")
        errors = "\n".join(validate_execution_receipt(value, expected_source_sha=SOURCE))
        self.assertIn("cycle", errors); self.assertIn("WIP", errors)

    def test_required_acceptance_cannot_be_waived(self):
        value = tracked_receipt(); value["project_work_kanban"]["work_items"][0]["checklist"][0].update(status="NOT_APPLICABLE", reason="skip")
        self.assert_rejected(value, "cannot be NOT_APPLICABLE")

    def test_optional_not_applicable_is_excluded_from_child_count(self):
        value = tracked_receipt(); value["project_work_kanban"]["work_items"][0]["checklist"].append({"id":"OPT-01", "text":"mobile build", "status":"NOT_APPLICABLE", "reason":"desktop-only tooling"})
        result = run_cli(value, "--render-markdown"); self.assertEqual(0, result.returncode, result.stdout)
        self.assertIn("(0 / 1)", result.stdout)

    def test_boolean_counters_are_not_zero(self):
        value = done_receipt(); value["project_work_kanban"]["work_items"][0]["must_fix_remaining"] = False
        self.assert_rejected(value, "must_fix_remaining", "--phase", "closeout")

    def test_renderer_does_not_emit_active_links_or_html(self):
        value = tracked_receipt(); value["project_work_kanban"]["work_items"][0]["title"] = "[click](https://example.invalid) <img src=x>"
        result = run_cli(value, "--render-markdown"); self.assertEqual(0, result.returncode, result.stdout)
        self.assertNotIn("[click](", result.stdout); self.assertNotIn("<img", result.stdout)
