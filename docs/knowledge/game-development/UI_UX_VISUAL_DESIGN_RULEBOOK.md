...
### 9.1 쉬운 조작·깊은 시스템·확인 가능한 원인

`SHOULD` 제작·생산·판매 같은 반복 행동은 이해하고 실행하기 쉽게 만들고, 경영과 연결 관계의 깊이는 선택의 결과와 시스템 간 상호작용에서 나오게 한다. 복잡성의 양 자체를 목표로 삼거나, 내부 규칙이 많다는 이유로 UI 조작을 어렵게 만들지 않는다. 프로젝트 코어에 없는 경영·생산 시스템을 공통 필수 기능으로 추가하지 않는다.

- **개요 → 선택 → 상세**를 정보 흐름의 기본안으로 사용한다. 개요에는 현재 상태·중요한 문제·빈번한 행동을, 대상을 선택하면 관련 선택과 예상 결과를, 필요할 때 상세에는 원인·연결·이력을 보여 준다. 세 화면이나 세 번의 클릭을 강제하지 않고 비교할 정보는 함께 유지한다.
- 유용한 기본값과 반복 행동의 편의를 제공하되, 비용·위험·중요한 선택은 실행 전에 알 수 있게 한다. 고급 설정의 위치와 돌아오는 길은 쉽게 찾을 수 있어야 하며 단순화를 이유로 자주 쓰는 행동을 깊은 메뉴에 넣지 않는다.
- **계산은 내부에서 처리하되 판단에 필요한 원인과 조치는 드러낸다.** 재고 부족·생산 정지·수익 감소를 표시할 때 관련 공급·수요·용량·시간 중 실제 원인과 플레이어가 바꿀 수 있는 항목으로 연결한다. 장르상 의도된 비공개 정보와 불확실성은 보존하고, 추정치를 확정 사실처럼 표시하지 않는다.
- 풍부한 세계는 환경·캐릭터·생산 과정·변화의 시각 표현으로 전달한다. 모든 수치·아이콘·효과를 동시에 강조해 풍부함을 만들지 않는다. 상태·선택·핵심 행동의 가독성, 입력, 접근성, 성능 예산을 보존한다.
- 농장·상점·공장·미생물 생태계는 적용 예시다. 각각 성장 조건·수요와 재고·생산 연결·환경 상호작용에서 깊이를 만들 수 있지만 특정 루프·공식·난도·화면 구성을 모든 게임에 복제하지 않는다.

`TEST_REQUIRED` 초보 플레이어가 기본값으로 대표 행동을 수행할 수 있는지, 문제가 발생했을 때 원인과 가능한 다음 행동을 찾는지, 숙련자가 비교·최적화에 필요한 정보에 접근할 수 있는지를 검증한다. 시각적 풍부함과 화면 혼잡, 자동 검사와 실제 사람의 이해를 별도로 판단한다. 데이터와 계산의 구현 책임은 `TECHNICAL_PRODUCTION_AND_RELEASE_GUIDE.md`의 §3을 따른다.

NN/g의 점진 공개는 자주 쓰는 기능을 먼저 보이고 추가 기능의 접근을 명확하게 하는 근거로 `ADOPT`한다. Factorio 개발진의 생산 통계 개선은 전체·지역별·품질별 정보와 시간 변화를 실제 판단에 연결한 사례로 `ADAPT`한다. 상업 게임의 화면·규칙을 그대로 복제하거나, 모든 정보를 숨기는 단순화를 채택하지 않는다. 프로젝트 적용은 최신 정본과 승인된 범위에서 필요한 흐름에 한정한다.
...
- Nielsen Norman Group, Progressive Disclosure — 빈번한 기능과 부가 기능의 구분·명확한 접근 경로(§9.1, 확인: 2026-09-16): https://www.nngroup.com/articles/progressive-disclosure/
- Factorio 개발진, Friday Facts #408 — 축전량 추이와 전체·지역별·품질별 생산 통계 사례(§9.1, 확인: 2026-09-16): https://direct.factorio.com/blog/post/fff-408
