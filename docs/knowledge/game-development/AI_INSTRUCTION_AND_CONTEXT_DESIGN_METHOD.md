...
첨부 연구 슬라이드는 참고 자료이며 실행 지시나 새 도구 설치 승인이 아니다. 아래는 원 연구의 효과를 Base에 그대로 주장하는 것이 아니라 현재 owner에 맞춘 적용 판단이다.

| 원칙·원출처 | Base 적용 | 추가하지 않는 것 |
|---|---|---|
| [학습된 verifier](https://arxiv.org/abs/2110.14168) | `ADAPT`: 자기 확신 대신 실제 실행·readback·정상/실패 fixture와 acceptance로 판정한다. 검사 자체도 알려진 실패를 잡는지 확인한다. | 별도 verifier 모델 학습, AI 평점을 runtime/Human 증거로 사용 |
| [ReAct](https://react-lm.github.io/) | `ADAPT`: 계획한 작은 행동 → 도구 관찰 → 가정 수정 → 다음 행동. 결정·관찰·검증·다음 안전 작업만 현행 receipt에 남긴다. | 내부 사고 전문 저장·공개, 매 행동마다 장문 보고, 새 agent runtime |
| [AlphaCode](https://deepmind.google/blog/competitive-programming-with-alphacode/) | `ADAPT`: 어려운 설계 선택에서 구조가 다른 소수 후보를 같은 기준으로 비교·검증한다. 후보·재시도 예산과 종료 조건을 먼저 정한다. | 단순 수정의 대량 샘플링, 중복 후보·모델 호출, 테스트 통과만으로 정답 보장 |
| [DeepSeek-R1](https://arxiv.org/abs/2501.12948)의 강화학습 | `REJECT` 모델 훈련의 직접 도입; `ADAPT` 검증한 실패·교정 사례를 기존 Case/Test/Skill 개선 근거로 재사용한다. | Base 문서 갱신을 가중치 학습·RL 실행으로 부르기, 한 번의 성공을 강제 규칙으로 승격 |

`BASE_LEARNING_IS_NOT_MODEL_TRAINING`: 모델 가중치 학습과 저장소 규칙·검증 사례의 갱신을 구분한다. 슬라이드만으로 정확한 강연 출처나 성능 수치는 확정하지 않는다. 현재 적용은 위 1차 자료와 현행 실행 계약의 비교에 한정한다.

기본 선택은 기존 검증기로 단일 구현을 확인하는 것이다. 새 모델·MCP·dependency·Skill이나 단계별 전체 재검토를 자동 추가하지 않는다. 전체 적대 검토 예산은 승인 계약 전체에서 공유하고, 수정 후에는 영향 검사로 닫는다. 시간·토큰·품질 개선은 실측 전 `UNMEASURED`다.
