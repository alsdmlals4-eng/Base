...
### 수정 가능한 콘텐츠와 복잡한 동작의 분리

반복해서 조정하는 콘텐츠 표는 **JSON 정본**, 복잡한 계산·상태 전이는 **검증 가능한 코드 모듈**로 나누는 것을 신규·개편 대상의 기본안으로 삼는다. 품목·가격·레시피·성장 조건·연결 관계 같은 선언적 값은 데이터가 소유하고, 생산 판정·경제 시뮬레이션·경로 탐색 같은 동작은 코드가 소유한다. 프로젝트에 없는 시스템을 이 예시 때문에 추가하지 않는다.

- JSON은 파싱 성공뿐 아니라 스키마 버전, 안정된 ID, 타입·필수 필드·기본값, 허용 범위, 중복 ID와 교차 참조를 검증한다. 오류에는 파일·ID·필드와 수정 방법을 남기고, 잘못된 데이터를 조용히 정상값으로 처리하지 않는다.
- 시작·콘텐츠 전환 등 명시된 로딩 경계에서 검증하고 메모리 모델·인덱스로 보관한다. 매 프레임 JSON 파일을 읽거나 다시 파싱하지 않으며, 개발 중 재로딩은 승인된 경계에서 캐시를 갱신한다.
- 선언적 설정은 등록된 동작 ID와 파라미터로 모듈을 선택한다. JSON 문자열을 임의 코드로 평가하거나, 콘텐츠 파일이 임의 스크립트 경로를 실행하도록 만들지 않는다. 알고리즘·불변식·오류 복구는 모듈과 그 테스트가 책임진다.
- UI와 통계는 같은 계산 결과를 소비한다. 계산 과정은 내부에 둘 수 있지만 플레이어에게 필요한 결과·주요 원인·가능한 조치는 조회할 수 있어야 한다. 정보 공개 기준은 `UI_UX_VISUAL_DESIGN_RULEBOOK.md`의 §9.1을 따른다.
- Scene·Theme·Animation·Texture와 엔진 자산 참조는 Godot의 native Scene/Resource/asset 구조를 유지한다. 같은 콘텐츠 값을 JSON·Resource·Script 기본값에서 각각 편집하지 않는다. Resource나 문서가 JSON에서 생성되는 경우 파생물로 표시하고 정본에서 재생성한다.
- 이미 안정적으로 운영하는 Resource 정본은 이 기본안만으로 재작성하지 않는다. JSON 정본과 native resource의 결합, 기존 Resource 정본 유지, 전면 JSON 변환을 비교해 편집자·실제 consumer·타입/Inspector 편의·검증·이관 비용에 맞게 선택한다. 전면 변환은 장기 이익이 입증되지 않으면 채택하지 않는다.
- 콘텐츠 버전과 저장 스키마 버전을 구분한다. 기존 저장이 참조하는 ID의 삭제·변경·의미 변경에는 호환성·마이그레이션·백업을 검토하고, export된 패키지에서 JSON 포함 여부·참조 자산·실제 로딩과 저장 복구를 확인한다. 에디터에서 파일을 읽은 결과만으로 패키지 검증을 대신하지 않는다.

이 구조는 편집 가능한 값과 동작의 책임을 분리하기 위한 Base 권장안이다. Godot 공식 [Resources 문서](https://docs.godotengine.org/en/stable/tutorials/scripting/resources.html)는 Resource의 직렬화·타입·Inspector·자산 공유 이점을 설명하며 JSON 사용을 의무화하지 않는다. 해당 이점을 보존하는 방식으로 `ADAPT`한다(확인: 2026-09-16).

프로젝트 적용은 최신 기획·데이터 owner·실제 consumer와 승인된 범위를 확인한 뒤 필요한 부분에 한정한다. 별도 프레임워크·범용 규칙 언어·중복 편집 정본을 추가하거나 기존 프로젝트를 일괄 전환하지 않는다. 새 콘텐츠 하나의 수정이 해당 consumer와 검증까지 이어지는지 확인하고, 문서 채택·정적 검사·runtime·사람 이해의 증거를 구분한다.
...
플랫폼 entitlement, app/build/request integrity, replay, server-authoritative value, offline/outage, false-positive recovery, privacy와 service sunset은 `docs/knowledge/game-development/GAME_ENTITLEMENT_INTEGRITY_AND_DRM_GUIDE.md`를 사용하고 프로젝트 증거는 `templates/project-operations/GAME_ENTITLEMENT_AND_INTEGRITY_RECORD.md`에 기록한다. Cloud Run Guide는 백엔드 적합성을, 이 Guide는 보호 신호와 플레이어 피해 경계를 소유한다.
