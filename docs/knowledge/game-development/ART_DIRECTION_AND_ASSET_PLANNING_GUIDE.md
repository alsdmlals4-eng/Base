...
기존 raster 후보의 레이어·프레임 정리, 상태별 미리보기 또는 스프라이트시트 내보내기가 실제 작업에 필요하면, fresh-read 뒤 **현재 연결된 Aseprite MCP를 우선 평가하고 적합한 경우 사용한다**. 모든 이미지 작업에 Aseprite를 강제하거나 모든 프로젝트에 설치하는 규칙은 아니다. 새 그림의 생성·창작 편집은 기존 이미지 모델 지침을 유지하며, Aseprite 사용 요청 또는 승인된 패키징 범위에서만 기존 픽셀의 구조화·내보내기를 수행한다.

1. 프로젝트 최신 AGENTS·채택 계약·Art Bible·승인 이미지·후보·Asset Catalog와 실제 consumer를 읽는다. 이 Base 지침을 읽었다는 이유로 프로젝트의 version lock이나 승인 자산을 교체하지 않는다.
2. 현재 세션에서 도구를 탐색하고 서버·실행 파일·허용 작업·출력 경계·버전·입력 규격을 확인한다. `INSTALLED`, `CONFIGURED`, `CLIENT_DISCOVERED`, `CALL_VERIFIED`, `TASK_VERIFIED`는 별도 증거 상태다. 설치·설정 기록만으로 현재 호출 가능하다고 보고하지 않는다.
3. native MCP가 실제 노출되고 필요한 작업이 허용되면 사용한다. 미노출 또는 필요한 작업/규격 미지원이면 승인·검토된 로컬 stdio 또는 CLI 경로가 현재 존재하고 동일한 안전 경계를 충족하는 경우에만 대안으로 사용하며 실제 transport를 기록한다. 오류·시간 초과 뒤에는 부분 출력과 재시도 안전성을 먼저 확인한다. 대안을 이용한 보안 거부 우회·권한 확대는 금지하며 대안 실행을 native 연결 성공으로 보고하지 않는다. 안전한 경로가 없으면 `BLOCKED_UNVERIFIED`와 부족한 기능을 보고하고 임의 설치·권한 확대·유료 실행을 하지 않는다.
4. 원본은 보존하고 현재 프로젝트의 승인된 후보 작업 경로 안에서 복사본을 처리한다. 다른 프로젝트·정본·경로 탈출·덮어쓰기를 허용하지 않으며 서버의 경계 검사만으로 OS sandbox가 있다고 주장하지 않는다. raw script 실행, 외부 전송 또는 과도한 입력이 필요하면 기존 허용 범위를 넓히지 않는다.
5. 실제 호출 뒤 source SHA-256, 도구 버전/transport, 레이어·프레임·상태 의미·duration·pivot/offset, export 설정과 결과 hash를 기존 작업 증거에 기록한다. 시트와 metadata 및 미리보기를 다시 읽고 필요한 상태 누락·투명도·정렬·crop을 확인한다. 정리 작업이면 유지하기로 한 원본 픽셀이 보존됐는지도 확인한다. 서로 다른 상태를 프레임으로 묶었다는 사실만으로 애니메이션 제작 완료가 되지 않는다.
6. 후보 생성, 사용자 승인, 정본 등록, 엔진 적용, 실제 인게임 화면 검증은 분리한다. `TASK_VERIFIED`는 해당 파일 작업의 검증일 뿐 runtime·Human PASS가 아니다. 사용자 선언 없이 Human 검수를 통과 처리하지 않는다. 불필요한 임시 출력은 소유권·참조·재현 증거를 확인한 뒤 기존 정리 규칙으로 제거하고 source·receipt·필수 증거는 보존한다.

예: 승인된 두 상태 이미지를 하나의 시트로 납품할 때는 실제 consumer의 크기·상태·정렬 계약을 따라 후보 시트와 metadata를 만들고 readback한다. 프로젝트별 규격을 공용 고정값으로 복사하지 않는다. 도구가 연결되지 않았거나 입력 규격을 지원하지 않으면 성공을 추정하지 않는다.

비교 근거: [diivi/aseprite-mcp](https://github.com/diivi/aseprite-mcp)는 Aseprite API 연결 대안이고 [공식 CLI](https://www.aseprite.org/docs/cli/)는 batch/export 대안이다. MCP는 연결된 세부 조작에 `ADAPT`, 승인된 CLI는 반복 export에 `ADAPT`, 수동 Aseprite 편집은 자동화 미지원 작업의 사용자 수행 대안으로 `ADOPT`한다. 새 중복 bridge 제작은 현재 필요가 없어 `REJECT`한다. 어느 대안도 설치·허용·현재 호출 검증을 생략하지 않는다.
