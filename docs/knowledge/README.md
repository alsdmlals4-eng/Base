...
## 5. 프로젝트에서 Base로 자동 승격하는 흐름

```text
프로젝트 문제 발생
→ 프로젝트 전용 해결안 적용·검증
→ 프로젝트 전용 내용과 공용 원칙 분리
→ 안정성·일반성 판단
→ Base 기존 규칙 또는 체크리스트에 자동 반영
→ 필요하면 cases에 구체 사례 기록
→ 반복 검증 뒤 methods 또는 skills로 확장
```

- 특정 프로젝트 이름 없이 설명 가능하고 반복 실패를 예방하는 안정적 작업 규칙은 Base 공용 규칙에 자동 승격한다.
- 검증되지 않은 관찰과 가설은 자동 승격하지 않고 상태를 명시한다.
- 한 프로젝트의 구체적 맥락과 결과는 필요할 때 cases로 기록한다.
- methods 또는 skills로 확장하려면 서로 다른 맥락의 반복 검증이나 강한 이론적·실증적 근거가 필요하다.
- 자동 승격은 기존 책임 문서를 직접 갱신하며 새 버전 복제본을 만들지 않는다.
