...
사례는 활성 프로젝트 기획서를 대신하지 않는다. 현재 수치·파일·Roadmap은 각 프로젝트 저장소에서 관리하며, Base 사례에는 문제 해결 원리와 검증 교훈만 남긴다.
