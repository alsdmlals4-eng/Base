...
Godot live-editor bridge는 기본 off, 최소 권한, loopback 전용, typed allowlist 방식으로 운용한다. 프로젝트가 transport·action·identity·도입 경계를 Manifest에 구성하기 전에는 `NOT_CONFIGURED`다. 원격 네트워크 공개, 임의 script 실행, 임의 shell 실행, 전체 파일시스템 접근을 기본 capability로 제공하지 않는다.
...
- 장기 작업이라는 이유로 mutation 승인·retry 위험이 사라짐
- timeout 뒤 실제 완료 여부를 모른 채 mutation·test·export 중복 시작
- task 결과를 다른 project·operation에 연결
- result hash가 다른 ledger 결과를 현재 task에 연결
- 동일 `capability_id`를 서로 다른 action에 중복 등록
- 등록되지 않은 capability를 project test runner로 지목
- screenshot·log·human evidence를 다른 검증 상태로 과장
- 경로 탈출로 프로젝트 밖 파일 수정
- engine input을 physical OS input으로 과장
- contract 파일 존재를 runtime·human PASS로 과장
- log·result에 secret 또는 절대 개인 경로 노출
- telemetry·외부 전송·제거 경로를 확인하지 않고 외부 도구 채택
...
`CONFIGURED` 상태에서는 다음을 비워 둘 수 없다.

- 실제 감지한 Godot 버전과 지원 최소·최대 범위
- 도구의 `source_type`, 원본 위치와 exact `version_pin`
- `telemetry_policy`
- `external_data_policy`
- 제거 절차와 `uninstall_reference`
- Git·snapshot·backup 등 `rollback_reference`

`DECLARED_REMOTE` 또는 `DECLARED_ALLOWLIST`는 전송 대상·데이터 종류·보존 기간·비밀 제거 방식이 프로젝트 보안 문서에 기록된 경우만 사용한다. 정책을 확인할 수 없으면 `NOT_CONFIGURED` 또는 `BLOCKED_SECURITY`로 유지한다.

## Typed action과 경로 제한

Capability Manifest의 `capability_id`와 arguments Schema가 allowlist다. 요청은 JSON Schema와 `tools/validate_godot_live_editor_contract.py`의 의미 검증을 모두 통과한 뒤 handler로 전달한다.
...
- 중복 `capability_id`는 `DUPLICATE_CAPABILITY_ID`로 fail closed한다.

## 효과 등급과 실행 방식

`effect_class`와 `execution_mode`를 분리한다.
...
`LONG_RUNNING_TASK`는 시간·resume 특성만 표현하며 승인이나 side effect 등급을 대신하지 않는다. `APPROVAL_REQUIRED_MUTATION + LONG_RUNNING_TASK`와 `NON_RETRYABLE_MUTATION + LONG_RUNNING_TASK` 모두 가능하다. 장기 작업은 항상 ledger와 `task_id`를 사용하고 automatic retry를 금지한다.
...
`APPROVAL_REQUIRED_MUTATION`과 `NON_RETRYABLE_MUTATION`은 실행 시간이 짧거나 길어도 사전 승인을 요구한다. 승인 후 project·action·argument·hash·`effect_class` 중 하나라도 바뀌면 `APPROVAL_TOKEN_MISMATCH`다.

token은 기본 single-use다. batch 승인은 같은 project, 같은 `effect_class`, 명시된 action 목록과 최대 항목 수에만 유효하다. AI는 automatic approval을 수행하지 않는다. JSON Schema는 token 모양을 검사하고 semantic validator는 token binding이 envelope top-level 값과 같은지 검사한다.

## Idempotency와 retry

`READ_ONLY + SYNCHRONOUS`만 기본 자동 retry 대상이다. `IDEMPOTENT_MUTATION`은 같은 idempotency key가 operation ledger에서 같은 normalized request와 결과를 가리킬 때만 retry할 수 있다.
...
- `effect_class`가 `NON_RETRYABLE_MUTATION`
- `execution_mode`가 `LONG_RUNNING_TASK`이고 기존 task가 존재할 수 있음
...
6. result binding과 result hash를 semantic validator로 확인한다.
7. idempotency 또는 resume 근거가 있을 때만 계속한다.
...
`execution_mode: LONG_RUNNING_TASK`는 start-once다. `task_id`가 만들어진 뒤 `status`와 `resume`은 같은 ledger/result store를 읽고 initiating action을 다시 전송하지 않는다.

- `QUEUED`, `RUNNING`, `PENDING`은 새 run 시작 금지
- `COMPLETED`, `FAILED`, `CANCELLED`, `STALE` 결과는 project·capability·operation·task binding 필수
- 다른 fingerprint·capability·operation·task 결과는 `TASK_RESULT_STALE`
- envelope `result.result_hash`와 ledger binding hash가 다르면 `TASK_RESULT_HASH_MISMATCH`
...
## Project test runner 무결성

`project_test_framework.state: CONFIGURED`인 경우 `runner_capability_id`가 catalog에 정확히 한 번 존재해야 한다. runner는 `CLI_HEADLESS` 또는 `EDITOR_PLUGIN`이어야 하고 `TEST_RESULT` evidence를 선언해야 한다.

- runner가 없거나 여러 개면 `PROJECT_TEST_RUNNER_NOT_DECLARED`
- `TEST_RESULT`가 없으면 `PROJECT_TEST_RUNNER_EVIDENCE_INVALID`
- 허용되지 않은 execution path면 `PROJECT_TEST_RUNNER_EXECUTION_PATH_INVALID`

Godot engine self-test, script parse, Scene load와 game behavior test를 한 상태로 합치지 않는다.
...
Evidence `kind`와 `state`는 Schema에 정의된 같은 영역의 조합만 허용한다. `HUMAN + CONTRACT_PASS`, `SCREENSHOT + PHYSICAL_INPUT_PASS` 같은 조합은 거부한다. screenshot과 viewport capture는 지정한 frame의 시각 출력만 증명한다. 사람이 이해하기 쉽거나 접근 가능하다는 결론은 `HUMAN_PASS` 증거가 필요하다.
...
| `DUPLICATE_CAPABILITY_ID` | 동일 capability ID가 둘 이상 존재 |
...
| `TASK_RESULT_HASH_MISMATCH` | terminal result hash와 ledger binding이 불일치 |
| `ENGINE_STATE_UNSUPPORTED` | 현재 import·play·debug 상태에서 action 금지 |
| `PROJECT_TEST_FRAMEWORK_NOT_CONFIGURED` | 프로젝트 테스트 runner 미등록 |
| `PROJECT_TEST_RUNNER_NOT_DECLARED` | configured runner가 catalog에 정확히 하나 존재하지 않음 |
| `PROJECT_TEST_RUNNER_EVIDENCE_INVALID` | runner가 TEST_RESULT evidence를 선언하지 않음 |
| `PROJECT_TEST_RUNNER_EXECUTION_PATH_INVALID` | runner 실행 경로가 프로젝트 테스트에 부적합 |
