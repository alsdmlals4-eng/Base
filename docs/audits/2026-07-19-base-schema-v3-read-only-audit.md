...
- Branch protection·Required Check 실제 설정: `[미검증]`
...
| Windows/Linux 생성 | Linux Actions | Linux 의존성 설치 | [부분 반영] | Ubuntu Actions 성공 | Windows LibreOffice·폰트·Poppler 미검증 |
...
- Windows 실제 생성: `[미검증]` — 현재 환경에서 LibreOffice가 PATH에 없음.
...
- Branch protection·Required Check 설정: `[미검증]`.
- Windows 생성·동일 입력 diff 0·실패 보존: `[미검증]`.
