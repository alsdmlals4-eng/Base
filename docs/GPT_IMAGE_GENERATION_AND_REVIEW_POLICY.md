...
coverage gap은 requirement이지 생성 권한 자체가 아니다. 다음을 구분한다.
...
예외는 새 artwork를 직접 authoring할 권한이 아니다.

```text
HOST_PLATFORM_PRECEDENCE
```

상위 host가 현재 메시지의 명시 요청을 요구하거나 tool call을 제한하면 상위 정책을 따른다.
...
## 7. Final lock, promotion and evidence ceiling
...
## 9. Long-term quality and automation
...
반복 승인은 줄이되 user final lock, product meaning, rights, cost, release, irreversible change는 자동 승인하지 않는다. 문제는 다음 계보로 남긴다.

```text
problem → reproducible evidence → root cause → correction → regression prevention → project readback → Base BCP
```

## 10. Actual post-change review
