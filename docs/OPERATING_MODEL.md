...
→ IMPLEMENTATION_CORRECTION_RESCAN
   ...
   └─ no required implementation/correction finding
      ...
      → minimum-five full-scope adversarial loops on the final candidate
      → CLEAN_REVIEW_EXIT
...
`POST_COMPLETION_ADVERSARIAL_REVIEW_REQUIRED`는 기존 최소 5회 full-loop + `CLEAN_REVIEW_EXIT` 규칙을 재사용하며 두 번째 독립 framework를 만들지 않는다. 마지막 구현·교정 이후의 최종 후보 상태가 이 검토의 입력이어야 한다. `NO_MATERIAL_FOLLOWUP`이면 횟수를 채우기 위해 가짜 finding이나 불필요한 변경을 만들지 않는다.
