...
정상 동작 중인 사용자 변경을 되돌리지 않는다. 실행하지 않은 검사·권한·도구는 통과로 보고하지 않는다. 외부 사례와 플레이어 반응은 요구사항 권한이나 구현 사실의 정본이 아니며 개선 가설의 근거로만 사용한다.
...
| 책임 | 실행 Skill |
|---|---|
| 요청 라우팅·사실 조사·사용자 확인·실행 계약·작업 분해·순서화 | `managing-project-intake-and-work-contract` |
...
활성 스킬은 모두 `load_by_default=false`이며 trigger가 일치할 때만 호출한다. 주 책임 분야 스킬은 최대 하나다. 검증·발행·Handoff는 해당 단계에서만 호출하거나 통합 스킬의 해당 mode로 실행한다.
...
5. 필요한 Skill과 mode·검증은 무엇인가?
6. 다음 작업·의존성·진입 조건은 무엇인가?
7. 외부 근거·플레이테스트·접근성·성능의 미검증은 무엇인가?
8. 미확정·보류·위험은 어디에 기록돼 있는가?
