...
→ IMPLEMENTATION_CORRECTION_RESCAN
   ...
   └─ no required implementation/correction finding
      ...
      → exactly two full-scope adversarial rounds
      → CLEAN_REVIEW_EXIT
...
`POST_COMPLETION_ADVERSARIAL_REVIEW_REQUIRED`는 **최종 후보 상태에서 수행하는 기존 `POST_CHANGE_MONITOR_LOOP` 자체**이며 두 번째 독립 review cycle이나 framework가 아니다. 그 동일 루프가 기존 정확히 2회 full-loop + `CLEAN_REVIEW_EXIT` 규칙을 충족한다. 마지막 구현·교정 이후의 최종 후보 상태가 입력이어야 하며, 이미 최종 후보를 대상으로 수행한 같은 full loop를 중복 계수하거나 다시 처음부터 반복하지 않는다. `NO_MATERIAL_FOLLOWUP`이면 횟수를 채우기 위해 가짜 finding이나 불필요한 변경을 만들지 않는다.
