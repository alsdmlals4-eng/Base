...
### 1.1 기본 설계와 상세 설계의 승인 경계

`BASIC_DESIGN_BEFORE_APPROVAL_DETAILED_DESIGN_AFTER_APPROVAL`

기본 순서는 다음과 같다. 이는 기존 `PLAN / BUILD / REVIEW` 안의 책임 구분이며 새 Work Mode나 별도 승인 시스템이 아니다.
...
| 경계 | 준비할 내용 | 완료 판단 |
|---|---|---|
| 승인 전: 기획 + 기본 설계 | 플레이어 가치·핵심 경험, 규칙, 범위·비목표, 프로젝트에 맞는 벤치마크, 와이어프레임·플로우맵, 필요한 대표 시각 후보·자산 상태군, 실제 consumer와 기존 구현 재사용, 구현 가능성·위험, 완료 기준·검증·롤백 | 사용자가 무엇을 왜 만들며 어떻게 작동하고 검증될지 판단할 수 있다. 적용되지 않는 항목은 이유를 남긴다. |
| 승인 후: 상세 설계 | 승인 의미를 유지하는 모듈·계약·씬·노드·리소스·스크립트 연결, 데이터·신호·저장 호환성, 자산 규격, 테스트와 구체 작업 순서 | 실제 구현 담당이 현재 저장소와 소비처를 재조회해 실행 가능한 명세로 구체화한다. |
| 구현 후: 검증 + 설명 | 실제 코드·테스트·실행 화면, 계획 대비 차이와 미검증 항목, 구현 연결도·설명 | 설계도 작성과 구현·실행·사용자 승인을 구별하고, 확인한 근거만 기존 블루프린트에 연결한다. |

- 승인 전에 상세 설계 전체나 모든 최종 자산을 먼저 제작하지 않는다. 다만 방향 판단·구현 가능성 확인에 필요한 세부 검토까지 미루지는 않는다. 조사로 해소되지 않는 기술 위험은 기존 `TECHNICAL_SPIKE` 규칙을 따른다. 승인 전 파일 변경·실행 실험은 **별도로 승인된 제한 범위**에서만 수행하고, 실험 성공을 제품 구현·플레이 경험 검증으로 승격하지 않는다.
- 승인 단위는 게임 전체 일괄 확정이 아니라 현재 `PLAY_MEANINGFUL_WORK_SLICE` 또는 범위가 명확한 변경이다. 이미 승인된 작은 수정은 관련 기존 기획·설계·검증을 재사용하며 전체 문서와 승인 절차를 매번 다시 만들지 않는다.
