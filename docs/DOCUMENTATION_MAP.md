...
→ 각 단계에 자동 선택된 최소 Skill과 Skill Mode
...
Work Mode → L1 이상 실행 작업의 PLAN → BUILD → REVIEW 순서와 현재 단계
...
L1 이상 작업은 PLAN·BUILD·REVIEW 각각의 실제 수행, 사용한 Skill·Skill Mode, 선택 이유, 결과·증거·미검증을 보고한다. PLAN 또는 BUILD에서 멈춘 작업은 완료가 아니라 현재 단계 상태로 보고한다. Skill 파일을 읽은 것과 실제 실행한 것을 구분한다.
