...
프로젝트의 사용자용 활성 본책은 게임 기획, 프로그래밍·MVP·로드맵, 아트, 사운드, QA·PM의 다섯 분야로 구성한다. `DESIGN_DOCUMENT_SYSTEM.md`는 이 다섯 본책과 세부 부록, 작업별 갱신 매트릭스를 정의한다.
...
사용자가 처음 확인할 활성 본책은 다음 다섯 문서다.

| 활성 본책 | 핵심 질문 |
|---|---|
| 게임 기획서 | 플레이어가 무엇을 왜 반복하며 어떤 규칙을 따르는가? |
| 프로그래밍 기획서 | 무엇이 실제로 구현됐고 MVP와 기술 로드맵은 무엇인가? |
| 아트 기획서 | 어떤 시각 품질과 최신 이미지를 기준으로 제작하는가? |
| 사운드 기획서 | 어떤 음악·효과음·음성이 경험을 전달하는가? |
| QA·PM 기획서 | 어떻게 검증하고 일정·위험·릴리스를 관리하는가? |
