...
→ 현재 작업의 Skill·mode·reference·Template·Test·Case
...
| 구분 | 파일 | 책임 |
|---|---|---|
| 최초 라우터 | `START_HERE.md` | 요청 유형별 최소 읽기와 실행 Skill·mode 연결 |
...
| Base Skill Registry | `skills/SKILL_REGISTRY.json` | 활성 Skill trigger·상태·경로·mode 라우팅 신호 |
...
| 작업 분해·의존성·병렬화·순서 | `managing-project-intake-and-work-contract` | 승인 뒤 `decompose-and-sequence` |
...
새 작업자는 저장소만으로 프로젝트 목적, 현재 단계, 보호 범위, 책임 원본, 실제 파일, 필요한 Skill·mode, 실행 순서, 외부 근거, 플레이테스트, 접근성·성능 검증, 다음 작업과 위험을 찾아야 한다.
