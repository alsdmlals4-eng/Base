...
활성 프로젝트·분야 본책은 구조화 JSON이 책임 원본이다. 활성 `*_기획서.md`, `DISCIPLINE_BIBLE.md`, `PROJECT_MASTER_PLAN.md`는 사용하지 않는다. 운영 라우터 Markdown은 유지할 수 있다.
...
| JSON 기획서를 DOCX/PDF·다이어그램으로 어떻게 발행하는가? | `docs/knowledge/methods/DISCIPLINE_PDF_PUBLICATION_METHOD.md` |
...
| JSON 기획서 DOCX/PDF 발행 | `publishing-discipline-bibles` | JSON·승인 이미지·생성기 변경 또는 stale |
...
모든 의미 있는 호출은 Learning Log에 결과·실패·예외·사용자 피드백·과다 호출·누락 검증·스킬 변경 필요성을 기록한다. 근거가 없으면 스킬 본문을 바꾸지 않고 `변경 없음`과 이유를 남긴다.
...
| JSON 기획서 원형 | `DESIGN_DOCUMENT.json` |
...
| JSON 기획서 발행 검사 | `github/check_design_document_publications.py` |
