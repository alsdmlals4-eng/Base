...
- GPT 비-Godot 완료→Codex 읽기 전용 Plan→단계별 Godot 구현→GPT 검수→`AGENT_MERGE_REQUIRED` Gate: `maintaining-project-context-and-handoff`의 `implementation-package-handoff` Mode.
...
- 실제 Prompt→Work Mode·Skill·Skill Mode 행동 평가: `evolving-project-discipline-skills`의 `behavior-eval` Mode와 `skills/SKILL_BEHAVIOR_EVALS.json`.
