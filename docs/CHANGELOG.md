...
## v1.10.0 - five-discipline project planning system

게임 프로젝트의 사용자용 활성 본책을 게임 기획, 프로그래밍·MVP·로드맵, 아트, 사운드, QA·PM의 다섯 분야로 정리하고 모든 작업에서 영향받은 본책을 같은 작업 안에 갱신하는 공용 계약을 추가했다.

변경:

- `AGENTS.md`와 공용 작업 규칙에 다섯 팀의 책임·경계와 작업별 문서 최신화 의무를 추가했다.
- 기획서 작성 스킬과 문서 체계 템플릿에 다섯 본책, 세부 부록, 최신 이미지 안정 경로와 10분 콜드 스타트 기준을 추가했다.
- Workflow와 MVP 체크리스트에 영향 분야 선택, 관련 본책·시각자료·검증 상태 갱신 절차를 추가했다.
- Documentation Map과 PR 템플릿에 다섯 본책 라우팅과 문서 영향 없음 근거를 추가했다.
- 프로젝트 고유 게임 규칙·수치·자산은 Base에 복사하지 않았다.
