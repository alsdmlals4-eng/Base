...
- [ ] L2 이상·다중 의존성 작업이면 `decompose-and-sequence`를 실행했다.
- [ ] 단계가 “코딩·문서 수정” 같은 활동이 아니라 독립 검증 가능한 결과다.
- [ ] `BLOCKS / INFORMS / USES_OUTPUT / SHARES_RESOURCE / VALIDATES` 관계를 구분했다.
- [ ] 병렬 작업의 입력·출력·파일 경계와 통합 지점이 명확하다.
- [ ] 각 작업 묶음에 진입 조건·통과 증거·실패·미검증 시 재계획이 있다.
- [ ] 가장 위험한 가설과 핵심 사용자 가치가 장식·후처리보다 앞선다.
...
- [ ] 필요한 외부 근거·플레이테스트·접근성·성능 범위를 판정했다.

## 4. 핵심 컨셉·벤치마크·플레이테스트·PoC
...
- [ ] 현재 결정을 바꿀 질문과 비교 차원을 먼저 고정했다.
- [ ] 직접 경쟁작·인접 장르·실패·혼합 사례를 구분했다.
- [ ] 공식 제품 사실, 플레이어 자기보고, 행동 이벤트·퍼널, 통제 실험, 해석을 분리했다.
- [ ] 긍정·부정·혼합, 최신·누적, 플레이타임·플랫폼·언어·패치 전후를 구분했다.
- [ ] 리뷰 폭탄·오프토픽·밈·복사 반응을 별도 표시했다.
- [ ] 플레이어가 제안한 해결책보다 문제·기대·상황·영향을 먼저 추출했다.
- [ ] 평점·판매량·강한 표현만으로 원인을 단정하지 않았다.
- [ ] 결과를 `ADOPT / ADAPT / AVOID / TEST / IGNORE`로 개선 결정에 연결했다.
- [ ] 출처·날짜·버전·표본·불확실성을 기록했다.

### 플레이테스트·실험·PoC

- [ ] 테스트 빌드·버전·대상 집단·기존 노출을 고정했다.
- [ ] 과제·플레이 시간·관찰 지점·피드백 채널을 명시했다.
- [ ] 관찰 행동, 이벤트·퍼널, 인터뷰·설문을 분리했다.
- [ ] A/B 테스트는 한 주요 가설, 통제군·변형, 사전 선언한 주 지표·가드레일을 가진다.
...
- [ ] 승인 범위와 현재 실행 단계에 직접 관련된 파일만 변경했다.
...
- [ ] 새 사실·실패·범위 변경이 생기면 이후 실행 순서를 재판정했다.
...
- [ ] 허용된 Legacy·Change Log·과거 case·plan과 실행 stale reference를 구분했다.
- [ ] 포맷·문법·타입·정적 검사
- [ ] 관련 자동 테스트
- [ ] 실제 런타임·렌더·빌드
- [ ] 핵심 정상 경로
...
- [ ] 핵심 플레이·입력·UI·정보 전달에 영향이 있으면 `accessibility-review`를 실행했다.
- [ ] 텍스트·대비·색상 외 표시·자막·오디오·진동 등 다중 정보 채널을 확인했다.
- [ ] 키·버튼 재지정, 홀드·연타·복합 입력 대안과 장치 전환을 확인했다.
- [ ] UI 탐색·포커스·탈출·오류 복구를 실제 경로에서 확인했다.
- [ ] 시간 제한·난이도·도움·일시정지·모션·깜빡임 장벽과 대안을 확인했다.
- [ ] 옵션 존재가 아니라 실제 적용을 확인했다.
- [ ] 결과를 법적 준수 인증으로 표현하지 않았다.

### 성능

- [ ] 렌더링·콘텐츠 부하·네트워크·플랫폼 영향이 있으면 `performance-profile`을 실행했다.
- [ ] 목표 하드웨어·OS·빌드·대표·최악 장면·예산을 고정했다.
- [ ] 평균 FPS뿐 아니라 frame time·hitch·spike·하위 구간을 확인했다.
- [ ] CPU·GPU·메모리·네트워크·로딩을 분리했다.
- [ ] profiler·debugger 오버헤드와 반복 측정 변동을 기록했다.
- [ ] 같은 조건의 baseline과 변경 후 capture를 비교했다.
- [ ] 성능 변경 뒤 게임 규칙·가독성·아트·접근성 회귀를 확인했다.

- [ ] 실행하지 못한 검증은 `UNVERIFIED`와 이유로 기록했다.
...
- [ ] 외부 근거·플레이테스트·실행 순서·접근성·성능 증거의 위치를 기록했다.
- [ ] Active Context가 실제 상태와 일치한다.
- [ ] 별도 Handoff를 두 번째 활성 상태 원본으로 만들지 않았다.
- [ ] 다음 작업·의존성·선행 조건·위험·롤백·읽기 순서가 있다.
...
- [ ] 실행 단계·의존성·게이트가 실제 완료 상태와 일치한다.
- [ ] 외부 근거·플레이테스트·접근성·성능의 한계와 미검증을 표시했다.
- [ ] 다음 작업·선행 조건·Active Context가 있다.
- [ ] 새 AI가 저장소만으로 방향·상태·책임 원본·Skill·mode·검증을 찾는다.
