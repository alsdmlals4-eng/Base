...
### 프로젝트 작업마다 사용하는 구현 학습 지도

`IMPLEMENTATION_LEARNING_MAP_EVERY_PROJECT_TASK`

프로젝트 작업은 시작·재개 시 관련 기능의 현행 관계도와 설명을 읽고, 종료 시 실제 변경과 비교한다. 기능·흐름·데이터·화면·상태가 바뀌면 같은 작업 안에서 해당 부분을 갱신해 사용자에게 보여준다. 영향 없는 오탈자·순수 정비 작업도 확인은 하되 `UNCHANGED_REUSED`와 근거를 남겨 기존 자료를 재사용한다. 매 응답마다 새 그림을 만들거나 모든 프로젝트·미착수 시스템을 일괄 역공학하지 않는다. 최초 적용 시 현재 작업의 필요한 지도부터 만들고 확인하지 않은 영역은 명시한다.

관련 기능 변화가 없는 순수 정비이고 재사용할 지도도 아직 없으면 `NOT_APPLICABLE — 기능 변화 없음, 기존 지도 없음`과 최초 관련 기능 작업의 준비 항목을 기록한다. 존재하지 않거나 읽지 않은 자료를 `UNCHANGED_REUSED`로 보고하지 않는다. 기능 변경에 필요한 지도 부재는 이 예외가 아니라 현재 작업의 준비 의무다.

담당은 기존 `building-project-visual-dashboards`와 프로젝트 분야 Skill이다. 시작 체크리스트가 이 절로 연결하고, 기존 Human Home·Blueprint·구현 명세 안의 동일 기능 ID를 사용한다. 별도 학습 앱·정본·진행률 원장은 만들지 않는다. 이 지도는 **학습 및 실제 수정·검수에 쓰는 읽기 전용 파생 자료**이며, 선을 바꾸는 것으로 코드·씬·정본을 수정하는 도구가 아니다.

각 변경 기능에는 다음 내용을 함께 제공한다.

1. **그림:** 한글 기능명과 관계를 가진 node/flow map. 화살표에는 호출·시그널·데이터 전달·상속 등 의미를 붙인다. 기능 전체 → 현재 작업 부분 → 필요한 구현 상세 순으로 나누고 모든 변수·함수를 한 화면에 펼치지 않는다.
2. **설명:** 무엇을 하는지, 왜 필요한지, 입력 → 처리 → 출력, 연결 대상, 정상·실패 예와 이번 변경 전/후를 쉬운 한국어로 설명한다. 기술 식별자는 상세 확인용으로 병기한다.
3. **실제 구현 연결:** 확인한 코드·씬·노드·Resource·데이터 owner와 검증 근거를 연결한다. scene tree의 부모/자식만으로 실제 호출 관계를 추정하지 않는다. 정적 확인, runtime 관찰, 기획 연결, 미확인 연결을 선 종류와 문자 legend로 구분하고 색만으로 전달하지 않는다. 동적 연결을 확인하지 못했으면 미확인으로 남긴다.
4. **화면 증거:** 시각·사용 흐름을 바꾼 기능은 실제 해당 build/scene/device의 화면 또는 전후 캡처와 설명을 붙인다. 비시각 기능은 실제 데이터 변화·테스트 결과를 관계도와 함께 설명하고 장식용 인게임 캡처를 강제하지 않는다. 캡처할 수 없는 필수 화면은 `NOT_RUN`과 blocker·다음 검증을 기록하며 합성 이미지로 채우지 않는다.
5. **사용·학습 안내:** 사용자에게 '어디서 이 기능을 사용하는가', '어떤 값/규칙을 바꾸려면 무엇을 요청하는가', '함께 영향을 받는 기능은 무엇인가', '어떻게 정상 동작을 확인하는가'를 제공한다. 임의 수정 권한을 부여하지 않는다.

기획 단계는 **어떻게 구현할 예정인가**, 구현 뒤는 **실제 어떻게 구현됐는가**로 같은 기능을 구분해 갱신한다. 계획 도식을 구현 증거로 바꾸거나 문서·자동 테스트 PASS를 runtime·사용자 학습 효과·Human 승인으로 승격하지 않는다. 사용자에게 매 작업 시험·퀴즈·수동 검수를 요구하지 않으며 Human 검증은 사용자의 명시적 선언 시에만 별도로 수행한다.

정확한 관계도는 기존 Markdown의 Mermaid/Flow 등 text-native source에서 렌더링한다. 이는 구조 정보의 시각 표현이지 생성 이미지 모델로 그린 코드 관계도가 아니다. 별도 아트 이미지가 필요한 경우 기존 이미지 승인 정책을 따른다. 사람용 Blueprint에는 **도식이 실제로 보이는 이미지/렌더 + 바로 옆 설명**을 포함하며 코드 블록이나 링크 목록만 전달하지 않는다. 현재 publication profile 안에 통합하고 source와 렌더를 별도 사실 정본으로 관리하지 않는다.

매 작업의 변경 부분은 같은 작업용 원본에서 렌더해 **기존 Human Home 또는 현재 응답 안의 미리보기 + 설명**으로 전달할 수 있다. 이 embedded preview는 별도 다운로드 묶음·새 산출물 정본이 아니며, 기존 승인 출력/임시 경로와 보존 규칙을 사용한다. 전체 PDF 재발행은 기존 milestone/publication Gate를 유지한다. Gate 사이에는 `작업 학습 미리보기 전달 완료 / 전체 PDF 개정 대기`를 분리하고, 최신 구현과 달라진 기존 PDF는 `STALE`로 표시한다. 해당 작업 자체가 PDF 발행 Gate이면 미리보기만으로 완료할 수 없다.

구현 대상 revision, 도식 source, 관련 파일, 캡처의 build/장면/기기와 검증 시점은 기존 AI-System 증거에서 추적한다. 기본 학습 화면에는 쉬운 상태 요약과 상세 근거 링크만 표시한다. 관련 구현이 바뀌면 해당 도식·설명·캡처의 유효성을 재검사하고, 낡은 자료는 `STALE`로 표시하거나 교체한다. 영향 없는 근거는 범위 비교 후 재사용할 수 있으며 새로운 revision에서 다시 실행했다고 주장하지 않는다. locked/승인된 역사 PDF는 덮어쓰지 않고 프로젝트의 개정·보존 규칙을 따른다.

완료 검증은 관련 기능의 누락, 선의 실제 근거, 계획/구현 구분, 한글 가독성·도식 잘림, source/출력 일치, 캡처 진위와 상태를 확인한다. 학습 자료가 필수인데 아직 준비되지 않았으면 그 작업을 완료로 표시하지 않는다. Godot 내 뷰어는 선택 사항이며 기존 addon의 버전·권한·라이선스·읽기 전용 가능성을 별도 프로젝트에서 검증한 뒤 채택한다. 이 공용 규칙은 addon 설치나 비주얼 스크립팅 전환을 승인하지 않는다.
