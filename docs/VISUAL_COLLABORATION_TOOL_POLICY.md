...
Dedicated Figma routing, Figma Bridge, localhost Expression/Sprite Studios, visual-delivery/project-management Tool Hub, QA Evidence Studio, external HTML workspace/catalog/dashboard는 active authority나 required project surface가 아니다. 재사용 가능한 아이디어—project identity, provenance, bounded edits, approval, versioning, reuse classification, readback, evidence ceiling, explicit handoff—만 current Notion/repository/PowerShell/Loop owner에 흡수한다.
