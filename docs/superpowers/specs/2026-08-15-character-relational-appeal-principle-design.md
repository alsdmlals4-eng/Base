...
현행 구조는 캐릭터를 “개별 인물의 욕망·판단·목소리·행동 방식”으로 구분하는 데 강하다. 그러나 **캐릭터 × 캐릭터 / 캐릭터 × 세계·배경 / 캐릭터 × 능력·역할의 조합이 새로운 선택·갈등·행동·대사·결과를 만들어야 한다**는 공용 원칙은 명시적으로 정의되어 있지 않다. 사용자 원문의 “능력간 시너지”도 누락하지 않기 위해 능력 축에는 능력끼리의 상호작용을 보는 선택적 보조 Lens를 포함한다.
...
> 캐릭터는 단독 설정표보다 관계와 상황 속에서 증명한다. 캐릭터 × 캐릭터, 캐릭터 × 세계·배경, 캐릭터 × 능력·역할의 조합에서 서로의 욕망·결점·강점·가치관·제약이 충돌하거나 보완되며 생기는 관계적 매력을 찾는다. 능력끼리의 결합 역시 실제 장면을 바꿀 때 같은 기준으로 본다. 좋은 조합은 설정을 더 붙이는 데서 끝나지 않고 새로운 선택·갈등·대사·행동·결과를 자연스럽게 발생시켜야 한다.
...
- 능력을 어떤 문제에 먼저 쓰는가
- 무엇에는 일부러 사용하지 않는가
- 능력에 실제 비용·한계·부작용이 존재한다면 이를 어떻게 받아들이는가
- 같은 능력이라도 가치관과 경험 때문에 다른 해결법이 나오는가
- 강점이 결점·책임·관계 문제를 새로 만드는가
- 능력 사용의 결과가 이후 관계·신념·자원 상태에 남는가

사용자 원문의 “능력간 시너지”는 이 축 안의 선택적 보조 Lens `ABILITY_X_ABILITY`로 보존한다.

```text
ABILITY_X_ABILITY
→ 여러 능력이 함께 있을 때 새 연계·선택·역할 분담·약점·예상 밖 결과가 생기는가
→ 실제 장면 차이가 없으면 설계하지 않음
→ 모든 능력 쌍의 조합표를 만들지 않음
```

이 보조 Lens는 네 번째 필수 축이 아니며, 능력 상호작용이 실제 장면을 바꿀 때만 사용한다.
...
- `CROSS_PRODUCT_OVERDESIGN`: 모든 캐릭터·능력 조합을 의무적으로 설계해 장면·콘텐츠 예산을 폭증시킨다.
...
1. `docs/knowledge/methods/NARRATIVE_AND_RELATIONSHIP_METHOD.md`
   - 공용 `RELATIONAL_APPEAL` 정의와 세 가지 주 조합 축, 선택적 `ABILITY_X_ABILITY` 보조 Lens를 추가한다.
...
AND THREE_PRIMARY_RELATIONAL_AXES_PRESENT
AND OPTIONAL_ABILITY_X_ABILITY_LENS_PRESENT
...
4. 능력에 존재하지 않는 비용·트라우마를 억지로 추가하게 만드는가?
5. 관계 태그나 호감도 수치가 실제 관계 경험을 대체하게 만드는가?
6. 선형 소설과 인터랙티브 게임의 매체 차이를 침범하는가?
7. 사용자 원문의 능력간 시너지를 `CHARACTER_X_ABILITY`로 잘못 치환해 누락하는가?
...
따라서 캐릭터를 프로필의 합으로 보지 않고 **서로와 세계와 능력 조합에 부딪힐 때 어떤 장면을 스스로 발생시키는가**를 작법과 검수의 공용 기준으로 만든다.
