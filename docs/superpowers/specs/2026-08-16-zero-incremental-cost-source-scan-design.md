...
- `AGENTS.md` for Base-wide invariant rules.
...
Do not add a new ACTIVE Skill, paid provider, local model dependency, daemon, second Source ledger, or duplicate Queue.

## Problem

The current scheduled Source Queue passes `${{ secrets.OPENAI_API_KEY }}` into `tools.periodic_source_analysis` and records `BLOCKED_MODEL_AUTH` when the secret is absent. That means the active design assumes a separately billed API path even though the user's standing constraint is that Base work must use only methods that create **no additional cost**.
...
This is a budget/authority gate, not a claim that every third-party service will remain free forever. Cost-sensitive external services must be rechecked before enabling a path that could charge money.
...
- pull-request write
- Actions dispatch write for downstream validators
...
The old `BLOCKED_MODEL_AUTH` path is removed from the active scheduler because model authentication is no longer a prerequisite for Queue preparation.
...
1. **Create an OpenAI API key.** Rejected because API/credit billing is a separate incremental-cost path.
...
5. **Keep API code in the active scheduled path but depend on a missing secret to block it.** Rejected because a future secret addition could silently re-enable incremental billing.

## Validation

TDD must require the active workflow/runner to contain `ZERO_INCREMENTAL_COST_REQUIRED` and `AWAITING_CHATGPT_REVIEW`, and to exclude `OPENAI_API_KEY`, `SOURCE_ANALYSIS_MODEL`, `python -m tools.periodic_source_analysis`, automated Source PR creation, downstream validation dispatch, and merge commands.

Repository-wide policy tests must bind `AGENTS.md` to the new invariant and require `COST_GATE_BLOCKED` for uncertain/metered execution.

Run exact-head Base v9 and full Game Project OS validation before merge, preserve unresolved review threads = 0, re-read current `main` before merge, and perform a post-merge Queue workflow readback.

## Rollback

Revert this change as one unit. Rollback restores the prior API-backed scheduled analysis path, so doing so would also restore the incremental-cost conflict and therefore requires a new user decision before re-enabling that behavior.