...
- Add one thin local Foundation adapter package to Registry v4 because active entries require local paths.
- The local package points to the pinned Base Skill and contains only Omenward canon roots, archive paths and validation commands; it does not copy the Base workflow body.
- Register the local ID `foundation.legacy-retention-archives` with `source_base_skill: governing-legacy-retention-and-archives`.
- Update the documented active Skill total from 12 to 13 while keeping count validation dynamic.
- Add the project archive adapter for `docs/design`, V2 canon, inactive compatibility Skills and CI evidence.
...
Implementation is split into five separate adoption PRs plus one final Base audit PR.

1. Base PR: Skill, templates, tests and cross-references.
2. Omenward PR: Registry v4 local Foundation adapter, archive adapter and validation.
3. Urban Legend PR: Registry v4 Base index route, archive adapter and active-reference tests.
4. Ten Paces PR: Registry v3 shared route, archive adapter and governance tests.
5. Blacksmith PR: adoption profile, archive adapter and audit validation.
6. Base audit PR: cross-repository rollout evidence and Learning Log closure.
