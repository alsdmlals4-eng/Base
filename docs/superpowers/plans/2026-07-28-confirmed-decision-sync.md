...
- [ ] **Step 1:** 공용 정책에 책임·우선순위·상태·정상 흐름·실패 흐름을 작성한다.
- [ ] **Step 2:** GitHub 댓글은 추적 증거, `CURRENT_CONFIRMED_DECISIONS.md`는 승인 결정 복원 원본, 분야 문서는 상세 규칙 원본, Sheets는 동기화 작업면으로 구분한다.
- [ ] **Step 3:** 문서 승인 직접 `main` 반영과 구현 PR 범위를 분리한다.
- [ ] **Step 4:** 승인 후 GitHub와 Sheets를 재조회해 `SYNCED`를 판정하도록 한다.
...
- [ ] **Step 1:** 프로젝트 약속·코어·보호 결정·현재 Decision 표·대체 관계·미결정·동기화 상태를 포함한다.
- [ ] **Step 2:** 상세 규칙 전문을 복제하지 않고 책임 원본 경로를 연결한다.
- [ ] **Step 3:** 마지막 GitHub main HEAD와 Sheet sync 정보를 기록한다.
...
- [ ] **Step 1:** 질문 전 대조 순서를 명문화한다.
- [ ] **Step 2:** `RECOMMENDED_DEFAULT`와 `USER_DECISION_REQUIRED` 분류를 추가한다.
- [ ] **Step 3:** 질문·승인 댓글, 정본·main·Sheet 갱신, 재조회, 실패 시 중단을 추가한다.
- [ ] **Step 4:** Decision Record에 정본 비교·PR·Sheet·sync 상태를 기록한다.
...
- [ ] **Step 1:** 병합 후 공격 렌즈에 최근 승인 누락, 정본 충돌, Sheet 불일치, 범위 외 변경, 중복 구현, 회귀를 추가한다.
- [ ] **Step 2:** 비판 검증 뒤 최소 수정과 재검사를 분리한다.
- [ ] **Step 3:** 표준 보고 템플릿을 추가한다.
...
- [ ] **Step 1:** 동일 책임의 기존 정책·열린 PR과 중복 여부를 확인한다.
