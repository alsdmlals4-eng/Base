...
사용자가 특정 branch/path가 다른 채팅에서 작업 중이라고 명시하면 그것 자체가 current-owner evidence이며, 해당 branch/path는 명시적 handoff 전까지 read-only다. 반대로 PR 상태만 open이라는 이유로 active worker를 추정하지 않는다.
