...
장기작업의 분야 owner는 의미와 계약을 책임지고 실행자는 실제 권한·도구 능력으로 선택한다. `docs/GPT_CODEX_WORKFLOW_POLICY.md`가 유일한 역할 정본이다. Work는 승인된 기획·제품 코딩·검증·교정·병합을 같은 작업에서 계속하며 앱 이름으로 강제 인계하지 않는다.

### 현재 작업 owner — 기획·운영

- Base 정책·Skill·Guide·Template·Learning
- Base Python test·CI contract·Registry/generated·Manifest·checker
- repository 정본; Notion은 명시적 V4 exception 또는 legacy migration만
- 기획·GDD·밸런스·데이터표·Flow·Storyboard
- 벤치마킹·시장/현업 조사·최소 3안 비교
- 적대적 검토·IRG
- 이미지 도구를 통한 후보 생성·편집·검수·repository 승인 delivery
- GitHub 비제품 문서/정본
- 문제→교훈→Base 승격
- 실제 Godot 구현 전 승인 의미·Acceptance 복원
- 실제 구현 결과 검수와 필요한 독립 검토

### 현재 허용된 실행자 — 제품 구현

현재 Work가 구현 능력을 갖추었으면 다음 제품 구현도 직접 수행한다. 능력·권한 부족 또는 사용자 지정 인계에만 다른 실행자를 사용한다.
...
Base test·Registry·generated·CI와 제품 코드는 필요한 책임을 구분하지만 실행 앱을 고정하지 않는다.
...
→ ONE USER APPROVAL
→ 현재 세션의 승인된 능력으로 NONCODING / PRODUCT BUILD
→ 필요한 능력이 실제로 부족할 때만 조건부 인계
→ repository exact-SHA / asset / engine identity 확인
...
`GPT_PRIMARY_IS_DECISION_OWNERSHIP_NOT_TEXT_ONLY`: 현재 세션은 prose-only가 아니다. 승인 범위의 문서·데이터·시각 후보·제품 구현·검증 인프라를 실제 가능한 도구로 수정·검증한다.

`REQUIRED_TOOL_EXECUTION_IS_NOT_OPTIONAL_EXECUTOR_HANDOFF`: 현재 세션에서 가능한 승인된 read/write/test/readback은 실제 수행한다. 불가능한 검증은 `NOT_RUN`으로 남기되 독립 구현까지 자동 중단하지 않는다. Codex 부재와 현재 세션의 능력 부족을 혼동하지 않는다.
...
## 6A. 기존 해결책 선보고·보존·재검토

### 적용 범위와 책임

이 절은 `REUSE_FIRST_PREFLIGHT_REQUIRED`와 `BETTER_ALTERNATIVE_SEARCH`의 실행 세부다. 시스템·생성기·공개 알고리즘·데이터/콘텐츠·UI/UX·에셋·도구·자동화·QA의 신규 제작 또는 의미 있는 개선에 적용한다. 시작은 `skills/managing-project-intake-and-work-contract/SKILL.md`, Godot 도구·자산의 적합성 및 도입 판정은 `skills/evaluating-godot-assets-and-plugins-before-creation/SKILL.md`가 계속 소유한다. 별도 Skill·후보 관리 시스템을 만들지 않는다. 프로젝트의 채택된 Base 계약·승인 결정·version lock을 최신 Base라는 이유만으로 자동 교체하지 않는다.

### 선보고와 기록

현재 프로젝트 구현·승인 자산/Reference → 기존 Base reuse/knowledge → 직접 관련된 프로젝트 evidence → 결정에 필요한 외부 자료 순서를 유지한다. 오픈소스·공개 알고리즘·공식 예제·플러그인·템플릿·에셋 중 요구와 실제 consumer에 맞는 유력 후보를 찾으면 **자체 제작·추가 개선 전에 사용자에게 먼저 보고**한다. 단순 링크 나열이 아니라 원출처·확인한 버전/시점, 해결하는 문제, 직접 사용/부분 흡수/참고/제외 판단, 자체 제작과의 차이, 호환성·라이선스·비용·의존성·유지보수·롤백·미검증을 요약한다. 무료·로컬·현재 연결된 도구를 먼저 비교하고 발견만으로 설치하지 않는다.

`considered_alternatives`, `rejected_alternatives`, `revisit_conditions`와 기존 Decision/작업 계약/자산 카탈로그의 해당 owner를 재사용한다. 후보의 원출처·제외 이유·당시 제약·승인 근거·다시 검토할 조건·최근 확인 근거를 연결하고, 자체 제작을 택했다는 이유로 후보를 삭제하지 않는다. 과거 결정을 덮어쓰지 않고 새 evidence와 후속 결정의 연결을 남긴다. 별도 후보 Registry·Notion/Sheets 복제·이중 정본을 요구하지 않으며 프로젝트 고유 후보와 채택 상태는 프로젝트에 남긴다.

### 재검토 트리거와 순서

다음 중 하나면 **다음 동일 방식 수정 전에** 기존 후보를 포함해 선택지를 재평가한다.

- 같은 실패가 반복되거나 완료 기준에서 측정 가능한 진전 없이 개선→검토가 반복된다.
- 작업 계약에 정한 시간·토큰·시도 예산 또는 재검토 조건에 도달한다. 관측하지 못한 사용량을 추정값으로 확정하지 않는다.
- 사용자가 다른 공개 방식·알고리즘·도구가 없는지 다시 묻는다.
- 새로운 증거·외부 구현·실패 원인이 생기거나 요구사항·플랫폼·호환성·라이선스·비용 등 기존 제외 근거가 달라진다.

예산이 없거나 사용량을 측정할 수 없어도 반복 실패·진전 정체·사용자 재질문은 독립적인 재검토 트리거다. 과거 후보와 제외 이유를 먼저 복원하고 → 그 이유의 현재 유효성을 확인하고 → 바뀐 제약에 필요한 범위만 추가 조사하고 → 현행 유지/부분 흡수/교체를 같은 기준으로 비교하고 → 새 근거와 권장안을 보고한다. 바꿀 이유가 없으면 현행 유지를 허용하되 그 근거와 다음 재검토 조건을 남긴다.

### 승인·채택 경계

**자체 제작 승인은 대안 탐색·보고의 영구 금지가 아니다.** 그러나 명시적 사용 금지 및 별도의 검색·재보고 금지, 보안·권한·비용·라이선스 제약은 임의로 해제하지 않는다. 범위가 금지된 후보를 몰래 시험하지 않는다. 후보를 다시 발견했다고 자동 설치·구매·교체하지 않는다. 이미 승인된 범위의 읽기·비교·가역적 검증은 반복 승인 없이 진행하되, 핵심 경험·구조적 정본·범위·비용·보안/권한·승인 방향을 바꾸는 전환은 `USER_DECISION_REQUIRED`다.

승인된 이미지·자산은 보존하며 발견/참고, 후보 생성, 사용자 승인, 정본 승격, 실제 적용, runtime 검증을 구분한다. 공개 자료라는 사실은 상업 사용·재배포 허가나 품질 검증이 아니다. 권리와 출처 검증은 기존 자산 평가 owner를 따른다.

### 반복 낭비 제한

트리거가 발생하면 과거 PASS나 REUSED_EVIDENCE만으로 재검토를 생략하지 않는다.

같은 승인 범위에서 scope·consumer·freshness와 제외 근거가 유효하면 `REUSED_EVIDENCE`로 재사용한다. 근거 변화가 없는 제외 후보는 반복 추천하지 않는다. 매 수정마다 전수 검색하거나 무관한 후보로 최소 대안 수를 채우지 않는다. L0 순수 기계 수정은 이유가 있는 `NOT_APPLICABLE`을 허용한다. 적용 대상의 조사를 수행하지 않았다면 `NOT_RUN`이며 `BUILD_NEW` 준비 완료로 표시하지 않는다.

검색 범위·원출처·실제 확인 결과를 기록한다. 후보를 못 찾은 것은 대안이 존재하지 않는다는 증명이 아니다. 필수 원출처를 읽지 못했으면 `BLOCKED_UNVERIFIED`로 남기고 제목·검색 요약·추정으로 효능을 확정하지 않는다. 사용자가 작업 근거로 직접 준 필수 링크의 접근 차단은 `AGENTS.md`의 중단 경계를 그대로 따른다.

### 비교 시험과 증거

시험이 필요하면 기존 결과물·실패 사례·승인 자산을 보존하고, 승인 범위의 격리된 시험에서 동일한 입력·제약·성공 기준과 측정 지표로 비교한다. 예를 들어 생성기는 목표 난이도 정의·해결 가능성·유효 결과 비율·생성 비용을 함께 보며, 외부 생성 결과만 가져오는 부분 흡수도 검토한다. 문서·정적 검사와 실제 생성/통합/런타임 검증을 구분하고 실행하지 않은 시험을 PASS로 쓰지 않는다. 사용자가 다시 묻기 전에도 트리거를 확인한 작업자가 후보 복원과 보고를 시작한다.

### 근거와 재사용 교훈

- OpenSSF, [Concise Guide for Evaluating Open Source Software](https://best.openssf.org/Concise-Guide-for-Evaluating-Open-Source-Software.html), 2026-09-06 확인: 후보를 요구·출처·유지보수·권리·의존성으로 평가하고 격리 시험한다는 원칙을 `ADAPT`한다. 인기도만으로 채택하거나 새 의존성을 무조건 추가하지 않는다.
- AWS, [Architectural decision record process](https://docs.aws.amazon.com/prescriptive-guidance/latest/architectural-decision-records/adr-process.html), 2026-09-06 확인: 거절 이유·결정 이력을 보존하고 새로운 근거로 후속 결정을 연결하는 원칙을 기존 repository owner에 `ADAPT`한다. 별도 ADR 시스템의 일괄 설치를 요구하지 않는다.

사용자 제공 사례의 생성기·소요 시간·해결 성과는 독립 검증하지 않았다. 공용 교훈은 “발견했던 대안을 한번 제외했다고 이후 진단 입력에서 없애지 않는다”이며, 특정 오픈소스의 우수성이나 시간 절감을 보장하지 않는다. 이 절과 문서 회귀검사는 절차의 보존을 검사할 뿐, 모든 AI 세션이 자동으로 검색·재평가를 실행하거나 모든 프로젝트가 이를 채택했음을 증명하지 않는다.
...
NONCODING 및 제품 BUILD는 같은 승인 계약과 현재 세션의 능력으로 수행한다. 별도 Work Instruction은 실제 인계할 때만 필요하다.
...
- 현재 세션의 가능한 Base·기획·제품 코딩·검증 작업을 계속한다.
- 필요한 능력·권한이 없을 때 해당 범위만 복구·조건부 인계한다.
...
- `HANDOFF_IS_NOT_DELIVERY`: 기획 종료는 구현 진입이며 전체 완료가 아니다. 현재 허용된 실행 경로에서 계속하고 실제 능력 부족 때만 인계한다. 권한·자료·검증이 차단되면 완료분·미완료분·정확한 blocker·재개 방법을 구분한다. 폐기된 로컬 호출 경로나 새 권한을 임의 복구하지 않는다.
...
같은 실패 명령을 맹목 반복하지 않는다. 반복 실패·진전 정체·대안 재질문은 §6A의 기존 후보 복원·재검토를 다음 동일 방식 수정 전에 수행한다.
...
```text
현재 세션의 이미지 도구로 candidate generate / edit / review
...
실제 이미지 도구와 consumer·승인 상태로 판단한다. 후보를 정식 자산으로 무단 승격하지 않는다. 도구/승인이 부족하면 `GPT_VISUAL_REQUEST`로 필요한 정보만 기록하고 독립 작업은 계속한다.
...
병합 뒤 새 main과 변경 owner·consumer·asset manifest·필요한 human projection·runtime evidence를 readback한다. 전체 적대 검토 회차를 다시 시작하지 않는다. 명시된 V4 Notion exception일 때만 해당 destination을 추가 확인한다. finding은 `POSTMERGE_CORRECTION_REQUIRED`로 표적 교정하고 `PROGRESS_READBACK_REQUIRED`로 남은 작업을 재계산한다. premerge 마지막 교정과 **same final-state lineage**를 유지한다.
...
> **현재 Work에서 승인된 기획·구현·검증·병합을 끝까지 수행하고, 실제 능력 부족 또는 명시적 지정 때만 필요한 범위를 인계한다.**
