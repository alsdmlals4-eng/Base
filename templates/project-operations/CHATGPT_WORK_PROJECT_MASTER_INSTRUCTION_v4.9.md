...
## 1. ChatGPT Work와 Base Work Mode는 다른 개념
...
## 13. Production Information과 이미지 분리
...
## 17. IMPLEMENTATION_READY / Codex / Engine Adapter
...
현재 기존 Godot 프로젝트는 `GODOT_DEFAULT_ACTIVE_ENGINE_ADAPTER`와 기존 Godot compatibility contract를 유지한다.
...
## 18. STABLE_ENGINE_BASELINE / local runtime
...
## 21. CANONICAL_REFLECTION_AFTER_PLAY
...
성능/용량이 관련되면 applicable download/install/patch size, memory, frame time, loading, CPU/GPU/network/mobile thermal을 실제 evidence로 측정한다.
...
OPEN_PR_READ_ONLY / current-task merge / CI_SUPPLY_CHAIN_GATE
...
Memory/past chat resume authority → DEFAULT_MEMORY_DISCOVERY_HINT_ONLY + FRESH_READ_PROJECT_BOOTSTRAP
explanation-image-first → TEXT_TABLE_FLOW_DB_FIRST + ACTUAL_CONSUMER_REQUIRED
copied Skill inventory → CURRENT_REGISTRY_IS_ROUTING_AUTHORITY
```
