...
> 이 문서는 **현재 승인된 결정만 빠르게 복원**하기 위한 프로젝트 운영 템플릿이다. 사람용 프로젝트 설명·결정 보기는 `NOTION_HUMAN_FACING_CANON`, 구조화 상태·Commit·실행 진실은 `REPOSITORY_STRUCTURED_CANON`을 따른다. Google Sheets는 `COMPATIBILITY_ONLY` migration source일 뿐 신규 입력이나 active sync 정본이 아니다.
...
## 2. Repository·Notion human view 동기화 상태
...
- 프로젝트 사람용 결정이 바뀌면 정확한 Project relation의 Notion record를 갱신하고 **destination readback**한다.
- 구조화 상태·Commit·실제 구현은 Repository가 소유한다.
...
6. 정확한 Project relation의 Notion 사람용 기록
...
→ 적용 가능한 Notion Project record 재조회
→ 구조화 정본과 사람용 기록 의미 일치 확인
→ SYNCED_TO_MAIN
```
...
| Notion destination readback | PASS / NOT_APPLICABLE / FAIL |  |
...
- `NOTION_HUMAN_FACING_CANON`과 `REPOSITORY_STRUCTURED_CANON`의 역할이 섞이지 않는다.
- 적용 가능한 사람용 변경은 Notion destination readback을 가진다.
