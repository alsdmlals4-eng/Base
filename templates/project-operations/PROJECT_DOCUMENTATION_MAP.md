...
| 책임 범위 | JSON 본책 | Skill Registry ID | 진입 스킬 | 실제 파일·자산 | 테스트·검증 | DOCX | PDF | Manifest | 상태 |
|---|---|---|---|---|---|---|---|---|---|
| 프로젝트 전체 |  |  |  |  |  |  |  |  |  |
| 설정·내러티브 |  |  |  |  |  |  |  |  |  |
| 게임 디자인 |  |  |  |  |  |  |  |  |  |
...
| 이미지·자산·VFX 변경 | 아트·테크니컬 아트 JSON | 자산 계약 | 아트·테크니컬 아트 | 발행·시각 검수 | Asset Manifest·실제 캡처 |
| 사운드 변경 | 사운드 JSON | 영향도 분석 | 사운드 | 오디오 QA·발행 | 이벤트·믹싱·반복 |
| 일정·범위 변경 | 프로덕션 JSON | 개발 게이트 | 프로덕션 | Handoff | Roadmap·Greenlight |
| 기획서 발행 | Design Document Registry·대상 JSON | publishing-discipline-bibles | 해당 분야 | 통합검수 | DOCX/PDF·해시·전 페이지 렌더 |
