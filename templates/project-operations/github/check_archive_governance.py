...
def validate_archive_governance(
    ...
    *,
    ...
    if require_pinned_commit and adapter["base"]["commit"] == "PIN_ON_PROJECT_ADOPTION":
        errors.append("project adapter must pin an exact 40-character Base commit")
    ...
    for active in paths["canonical_sources"]:
        ...
        try:
            ...
            errors.append(f"canonical source is inside archive root: {active}")
    ...
    for record in manifest["records"]:
        ...
        if archive_id in seen_ids:
            errors.append(f"duplicate manifest archive_id: {archive_id}")
...
def main() -> int:
    ...
    parser.add_argument("--allow-template-pin", action="store_true")
    ...
    errors = validate_archive_governance(
        ...
        root / args.adapter,
        root / args.manifest,
        require_pinned_commit=not args.allow_template_pin,
    )
    if errors:
