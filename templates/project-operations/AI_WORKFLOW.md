...
`PM_EXECUTION_GATE_REQUIRED`: `WORK_PROJECT_START_CANON_CHECKLIST.md` §12.1의 root JSON을 사용하고 기존 Issue/카드와 같은 receipt를 갱신한다. 전체 작업·완료/필수 수·현재 작업·차단·다음 행동을 실제로 표시한다. 다음 승인 작업을 먼저 active로 선택·기록한 뒤 `--phase resume` 검사, 모든 필수 작업 완료 후 `--phase closeout` 검사를 수행한다. trusted source와 Markdown 출력 옵션은 유지한다. 차단 목록 표시는 실행 승인과 다르며 nonzero 종료를 PASS로 바꾸지 않는다.
