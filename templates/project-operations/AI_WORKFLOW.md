...
사용자는 만들고 싶은 결과와 중요한 방향을 설명한다. 저장소 운영체계가 11개 분야 판정, Markdown·JSON 책임 원본, trigger 기반 최소 스킬 호출, 개발 게이트, 사람용 최신 PDF, 검증과 인수인계를 기억한다.

```text
사용자 방향
→ Registry 기반 분야·최소 스킬 라우팅
→ 저장소 사실 확인·딥인터뷰·사용자 마지막 확인
→ GPT 문제·경험·영향도 구조화와 실행 프롬프트
→ Definition of Ready
→ 사용자 승인
→ Codex 실제 저장소 조사·Plan
→ Implementation
→ 자동·수동 Verification
→ 관련 Markdown 또는 JSON 책임 원본·실제 상태 갱신
→ PDF·Manifest와 선언한 선택 DOCX/다이어그램 재생성
→ QA·통합검수
...
- 저장소에서 확인 가능한 사실을 먼저 조사하고 사용자 결정만 질문
- 주 책임·영향 분야·변경 유형 판정
- Design Document Registry에서 관련 Markdown 또는 JSON 책임 원본 선택
- Skill Registry에서 최소 스킬 선택
- 확정·권장·확인 필요·보류·미검증 분리
- Markdown 또는 JSON 기획 초안·Decision·Roadmap·Gate·Work Order 작성
...
- 최신 지시·Design/Skill/Interview Registry·관련 책임 원본·실제 파일 확인
- 호출할 최소 스킬과 후속 스킬 판정
- 사용자 가치·보호 범위·미확정 기록
...
- 관찰 가능한 완료 기준
- 자동·수동·사용자 검수
- 관련 Markdown/JSON 책임 원본·스킬·발행 영향
...
- 실패·폴백·롤백
- Acceptance Criteria
- Markdown/JSON 책임 원본·Registry·Skill·Learning Log·PDF/Manifest와 선택 파생본 갱신 계획
- 사용자 승인
...
```text
Skill Registry 갱신
→ 필수 PROJECT_SKILL_MAP.pdf + 선택 md/docx/assets 생성
...
→ 보존하거나 승인할 Markdown/JSON 책임 구조와 사람용 발행 구조 제안
→ 변경 전후 보존 대조
→ 사용자 승인
→ 승인 범위만 승계
