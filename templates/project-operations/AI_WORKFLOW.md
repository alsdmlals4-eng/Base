...
`PROJECT_START_CANON_CHECKLIST_REQUIRED`: L1+ 작업은 Project AGENTS·exact implementation·`skills/PROJECT_BASE_ADAPTER.json`의 Base pin을 fresh-read·validation한 뒤 `WORK_PROJECT_START_CANON_CHECKLIST.md`를 먼저 실행한다. project repository가 소유한 같은 receipt에 `benchmark_preflight_receipt`, context hygiene inventory와 root `project_work_kanban`을 작성하고, adapter validation으로 확인한 exact Base checkout에서 `python <resolved-Base-root>/tools/validate_work_contract_receipt.py --receipt <project-repository-owned-receipt.json> --phase start --expected-source-sha <fresh-read-project-source-sha> --render-markdown`을 실행한다. 기대 SHA는 receipt 자체가 아닌 신뢰한 caller의 별도 fresh-read 결과다. 실패하거나 Base root/pin을 해석하지 못하면 `BLOCKED_UNVERIFIED`로 두며 기획·제작·구현을 진행하지 않는다.

`PM_EXECUTION_GATE_REQUIRED`: `WORK_PROJECT_START_CANON_CHECKLIST.md` §12.1의 root JSON을 사용하고 기존 Issue/카드와 같은 receipt를 갱신한다. 전체 작업·완료/필수 수·현재 작업·차단·다음 행동을 실제로 표시한다. 다음 승인 작업을 먼저 active로 선택·기록한 뒤 `--phase resume --expected-source-sha <fresh-read-project-source-sha> --render-markdown` 검사를 수행하고, 모든 필수 작업 완료 후 `--phase closeout --expected-source-sha <fresh-read-project-source-sha> --expected-head-sha <fresh-read-final-head-sha> --render-markdown`으로 마감한다. `TRUSTED_VERIFICATION_TARGET_HEAD`는 receipt 내부 값이 아니라 신뢰한 caller가 별도로 fresh-read해 전달한다. 차단 목록 표시는 실행 승인과 다르며 nonzero 종료를 PASS로 바꾸지 않는다.

이 gate는 현재 project consumer에 맞는 ADOPT/ADAPT/REJECT 판단과 안전한 정리, PM 기록 일관성을 요구한다. Base가 특정 게임의 장르·세계관·메뉴·버튼·wireframe·수치를 고정하는 규칙은 아니다.
