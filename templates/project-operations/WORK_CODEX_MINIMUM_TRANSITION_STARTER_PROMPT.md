...
`PM_EXECUTION_GATE_REQUIRED`: `WORK_PROJECT_START_CANON_CHECKLIST.md` §12.1의 root JSON과 `PROJECT_WORK_ITEM_CHECKLIST.md` §10을 사용해. 기존 Goal의 필수 전체 작업·현재 항목·완료/필수 수·차단·다음 행동을 실제로 보여 줘. 작업 전환 때 다음 승인 작업을 먼저 active로 선택·기록한 뒤 `--phase resume`, 마감 때 `--phase closeout`으로 같은 검사기를 실행하고 trusted source·출력 옵션을 유지해. 올바른 차단 목록은 정보용으로 표시해도 실행은 nonzero로 유지해. 새 PM 앱이나 빈 병렬 상태 문서를 만들지 마.
