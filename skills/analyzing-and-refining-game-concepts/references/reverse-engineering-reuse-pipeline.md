...
이 Reference는 `analyzing-and-refining-game-concepts`의 벤치마킹 작업에서 장르·메커닉뿐 아니라 **시스템, 콘텐츠·데이터 구조, UI/UX, 도구, 자동화, 에셋·이미지 재료, 작업구조, Skill/Eval, QA 패턴**까지 재사용 후보를 능동적으로 발굴할 때 사용한다.

상위 공용 계약은 `docs/BENCHMARK_REVERSE_ENGINEERING_AND_REUSE_PIPELINE.md`를 따른다.

## 핵심 계약

`PROJECT_REUSE_OPPORTUNITY_SCAN`은 사용자가 이미 알고 있는 예시 목록을 늘리는 절차가 아니다. **대상 프로젝트의 최신 정본과 실제 병목을 먼저 읽고, 그 문제를 해결할 재사용 후보를 외부 사례와 기존 내부 자산에서 역으로 찾는 절차**다.
...
`EXAMPLE_IS_NOT_SCOPE_LIMIT`: 테트리스류, 선택형 비주얼노벨/텍스트 로그라이크, 덱빌딩, 서바이버라이크 등 사용자가 직접 든 사례는 탐색 seed일 뿐 고정 범위가 아니다. 프로젝트의 실제 문제와 제작 비용을 더 잘 줄이는 후보가 있으면 다른 장르·제품·툴·오픈소스·에셋·제작 파이프라인·QA 방식까지 조사한다.

## PROJECT_CANON_FIRST

역공학 후보를 찾기 전에 최신 프로젝트 정본과 실제 코드·데이터·Scene·Resource·자산·테스트에서 다음을 확인한다.
...
후보는 유명세보다 현재 병목을 얼마나 줄이는지로 평가한다.
...
| Mechanic / system | 입력·상태·규칙·결과 | 콤보, 상태효과, 드래프트, 리롤, 위험/보상 |
...
단일 작품의 고유 조합을 그대로 “장르 공식”으로 승격하지 않는다.
...
이 Reference는 후보를 발굴하고 추상화하는 owner다. 최종 권위는 기존 owner에 넘긴다.

- 게임 규칙·시스템·콘텐츠 → 프로젝트 기획 정본과 해당 설계 owner.
...
- Workflow → 기존 Base 운영 policy/reference/mode에 먼저 흡수.
- Skill/Eval → 기존 Skill adoption/evolution owner.
- Testing/QA → 현재 validation owner.
- 프로젝트 고유 후보 → `PROJECT_ONLY`.
...
실제 프로젝트 스캔은 `templates/research/PROJECT_REUSE_OPPORTUNITY_SCAN.md`에 기록한다.
