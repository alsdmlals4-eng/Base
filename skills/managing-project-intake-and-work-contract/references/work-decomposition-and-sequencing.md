...
공식 자료 비교(2026-09-01 확인): Godot [Scene organization](https://docs.godotengine.org/en/stable/tutorials/best_practices/scene_organization.html)의 단일 목적·느슨한 결합은 `ADOPT`, [Project organization](https://docs.godotengine.org/en/stable/tutorials/best_practices/project_organization.html)의 관련 파일 배치는 프로젝트 기존 경로에 맞춰 `ADAPT`한다. 파일 종류별 분리만으로 기능 경계를 대신하는 방식과 모든 기능의 독립 패키지화는 기본안으로 `REJECT`하며, 실제 배포·소비·검증 필요가 생길 때만 재검토한다.
