...
외부 운영 사례가 workflow/tool/Skill/evaluation/QA/content-pipeline 결정을 바꾸는 경우에만 같은 entry에 `operator_case_reconstruction`을 추가한다. six fields는 `input_anchor_and_source`, `staged_execution_and_handoffs`, `human_decision_and_approval_boundary`, `observable_outcome_and_evidence_ceiling`, `nontransferable_or_unobserved`, `project_trial_and_acceptance_gate`이며 모두 nonempty여야 한다. 이 조건부 block은 public 사례를 직접 runtime·생산성·project adoption proof로 바꾸지 않는다. 원출처가 읽히지 않거나 input/단계/handoff가 관찰되지 않으면 그 한계를 기록하고, project-specific 최소 trial 및 기존 human acceptance Gate를 유지한다. 실제 execution observation·failure·next action은 새로운 정본을 만들지 않고 `skills/optimizing-ai-model-and-prompt-costs/references/model-stack-routing.md`의 `execution_strategy`에 둔다.
