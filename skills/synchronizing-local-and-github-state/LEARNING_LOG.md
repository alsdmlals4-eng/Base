...
- **Finding:** local/remote ahead·behind와 textual path overlap만으로는 안전한 write·PR·merge를 증명할 수 없다. 다른 파일이 같은 Canon·Schema·generated derivative·Scene·asset family를 변경할 수 있고, 현재 PR 자체를 same-goal duplicate로 오인하거나 첫 write parent와 최종 reviewed head를 같은 SHA 의미로 섞을 수도 있다.
