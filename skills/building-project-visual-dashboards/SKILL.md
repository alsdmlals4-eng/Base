...
description: Use when a project needs a self-contained repository-owned human projection or Visual Map that explains concepts, loops, systems, UX/visual state, evidence, risks, and next work without replacing repository truth. A Notion surface is only an explicitly approved V4 exception.
---

# Building Project Visual Dashboards — Repository Human Projection & Visual Map
...
이 Skill은 사람이 프로젝트를 **추가 페이지 이동 없이 메인 Home 한 화면에서 이해**할 수 있도록 repository의 human-facing Markdown/PDF projection과 Visual Map을 구성·갱신한다. Notion은 명시적으로 승인·범위가 정해진 V4 예외일 때만 파생 destination으로 사용한다.

Repository Markdown/JSON/code/scene/resource/test/runtime evidence를 복제 정본으로 만들지 않는다. repository의 exact-SHA human projection은 사람이 이해·비교·학습하는 derived view이고, structured/runtime truth는 Repository가 계속 소유한다.
...
Project Home을 만들거나 교정할 때는 `docs/operations/HUMAN_HOME_SELF_CONTAINED_POLICY.md`와 current-project benchmark receipt의 현행 계약을 실행 기준으로 사용한다. 제목·짧은 정의 다음의 상단 핵심 구간에는 프로젝트별 **설명형 Visual GDD**를 장식 이미지보다 우선하고, 모든 프로젝트에 동일한 이미지 세트나 taxonomy를 강제하지 않는다. 승인된 Flow/System Map·UI/Visual Guide·프로젝트 고유 핵심 표처럼 사람이 판단해야 하는 현재 작업물은 단순 링크 제목으로 숨기지 않고 repository human projection에서 실제 내용을 볼 수 있게 한다.

Human projection을 정리한다는 이유로 AI 구현·검증 세부정보를 삭제하지 않는다. repository AI-System surface에는 schema·ID·source mapping·implementation/sync state·evidence·validation/test/QA·PR/commit/handoff metadata를 보존하고, `Human Projection ↔ Detail Canon ↔ AI-System surface ↔ Repository/runtime evidence` 추적성을 유지한다. 최종 판정은 **human projection만으로 무엇을 만들고 어떻게 만들지 판단 가능하며, AI-System surface만으로 구현·검증 세부정보가 부족하지 않은가**를 기준으로 한다.
...
```text
설명/배치만 수정
→ repository human projection update + exact-SHA readback

게임 규칙/기획 수정
→ 영향 조사 → 변경 전/후/기대효과 → 사용자 승인
→ repository structured canon sync
→ repository human projection sync/readback → 구현/검증
...
- `frame-project-home`: exact Project identity, latest confirmed decisions, GitHub main, existing repository human projection, 관련 Flow/System/Visual/사람용 표와 현재 blocker를 읽어 이번 Home의 범위와 보호 대상을 고정한다. 명시된 V4 Notion exception만 추가로 readback한다.
...
- `build-project-home`: 핵심 이해가 자체 완결된 repository human projection을 구성·갱신한다. `30초 전체 그림 → 5분 핵심 Flow/System/Data/Visual → drilldown` 순서로 배치하고 관계 설명에 실질적으로 필요할 때만 Visual Map/diagram을 추가한다.
...
→ latest GitHub main + exact-SHA repository human projection readback; V4 Notion exception only when applicable
...
- repository human projection and repository Detail Canon; explicitly scoped V4 Notion exception only when applicable
...
- self-contained, information-rich repository human projection
...
과거 HTML dashboard의 UNIQUE 자료가 있으면 `docs/DEPRECATED_PROJECT_SURFACE_RETIREMENT_POLICY.md`에 따라 현행 repository owner와 human projection으로 흡수한다.
...
- repository human projection 주장과 repository/runtime evidence가 충돌하지 않는가; V4 Notion exception이 있으면 destination도 동일하게 검사했는가
...
기존 human-facing 정보 계층 원리는 `skills/building-project-visual-dashboards/references/dashboard-information-architecture.md`를 참고하되, standalone HTML 탭 구현이 아니라 repository human projection/Visual Map에 맞게 적용한다.
