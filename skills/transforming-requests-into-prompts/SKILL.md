...
이 스킬은 딥인터뷰를 대신하지 않는다. 기능·경험·아트 방향·구조·워크플로 변경은 먼저 `conducting-deep-requirement-interviews`에서 저장소 사실, 모호성 장부, 마지막 재진술과 사용자 확인을 완료한다. 인터뷰가 필요한데 `CONFIRMED`가 아니면 실행 프롬프트를 만들지 않는다.
...
- 사용자 원문
- 해당하는 경우 `CONFIRMED` 인터뷰 기록과 `user_confirmation_ref`
- 프로젝트 기준 문서와 현재 상태
- 기존 결정과 보호할 동작
- 작업의 사용자·플레이어 가치
...
1. 원문에서 명시 요구를 추출한다.
2. 딥인터뷰 대상이면 Registry 상태와 사용자 확인 근거를 검사한다. 미확정이면 중단한다.
3. 해결하려는 문제와 필요한 경험을 분리한다.
4. `[확정]`, `[권장]`, `[확인 필요]`, `[제외]`, `[보류]`로 상태를 구분한다.
5. `DOCUMENTATION_MAP.md`를 따라 관련 기준 문서와 영향 파일만 확인한다.
6. L2 이상이면 인터뷰에서 확정된 대안을 비용·위험·장점과 함께 보존한다.
7. 선택한 방향을 아래 8요소로 작성한다.
8. 실행자에게 필요한 읽기 순서, 위험, 보고 형식을 추가한다.
9. 구현·검증·문서 최신화를 서로 다른 단계로 분리한다.
...
- 이미 확정된 사항을 반복 질문
- 인터뷰의 제외·보호·보류·미검증 항목을 누락하거나 새 요구를 발명
- 사용자 확인 전 실행 프롬프트 생성
