...
| 작업 | 이 Skill | 다른 Skill |
|---|---|---|
| 기존 프로젝트의 코어 사실 판정 | 입력으로 사용 | `identifying-project-core` |
| 핵심 컨셉·뾰족한 재미·제약·PoC 탐색 | 후보를 소비 | `analyzing-and-refining-game-concepts` |
| 코어 제안·경계·불변 조건·승인 계약 | 책임짐 | - |
| 기획서 원본 작성·Registry·발행 | 승인된 계약 제공 | `managing-design-documents` |
| 실제 구현 변경 검증 | 코어 회귀 기준 제공 | `reviewing-and-validating-project-changes` |
| 변경안 공격·비평·개선 | 확정 전후 스트레스 테스트 | `running-adversarial-review-and-refinement` |

## Work Mode and Skill Modes

이 Skill의 기본 Work Mode는 `PLAN`이다.
...
- `reopen`: 새 근거 또는 명시적 방향 변경 요청이 있을 때 코어 재검토를 시작한다.
...
- `CORE_STRESS_TESTED`: 제거·대체·실패 반례를 통과하거나 문제가 기록됐다.
...
### 2. Write the minimum core proposal
...
### 3. Separate invariants and changeable shell
...
2. 중심 시스템을 다른 방식으로 대체해도 정체성이 유지되는가?
3. 플레이어가 위험·선택·피드백을 느끼지 못해도 핵심 재미가 남는가?
4. 코어가 너무 넓어 일반적인 장르 설명이 되지 않았는가?
5. 코어가 너무 좁아 하나의 기능 구현 방식에 종속되지 않았는가?
6. 기술 부채나 현재 임시 구조를 불변 조건으로 고정하지 않았는가?
...
침묵·추정·이전 대화의 모호한 긍정을 승인으로 사용하지 않는다.
...
확정된 코어는 영구 불변이 아니지만 일반 기능 수정과 같은 방식으로 바꾸지 않는다.

코어 변경은 다음 조건에서만 `reopen`한다.

- 사용자가 프로젝트 방향 변경을 명시했다.
- PoC·플레이테스트가 핵심 가설을 반복 반증했다.
- 제작·기술·법적 제약으로 핵심 경험이 성립하지 않는다.
- 현재 구현이 코어와 장기간 충돌해 프로젝트 정체성이 이미 변했다.
- 기존 코어 문장끼리 모순된다.
...
- 프로젝트 목표와 대상 플레이어가 있다.
- 핵심 컨셉·행동·루프 후보가 있다.
- 제약과 MVP·PoC 목표가 있다.
- 기존 프로젝트라면 `identifying-project-core` 결과가 있다.
- 승인 권한과 기록 위치가 명확하다.
...
- 적대적 스트레스 테스트: `running-adversarial-review-and-refinement`
- 문서화·발행: `managing-design-documents`
- 구현·회귀 검증: `reviewing-and-validating-project-changes`
