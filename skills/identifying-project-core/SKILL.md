...
description: Use when an existing game or software project needs an evidence-based determination of which experiences, loops, systems, rules, visual directions, or technical foundations are identity-defining project core rather than replaceable content, MVP support, or implementation detail.
---
...
중요해 보인다는 이유, 개발 비용이 컸다는 이유, 현재 구현되어 있다는 이유만으로 코어가 되지는 않는다. 현행 책임 원본, 사용자 승인 결정, 실제 코드·데이터·자산·테스트와 의존 관계를 근거로 판정한다.

## Distinguish

| 작업 | 이 Skill | 다른 Skill |
|---|---|---|
| 기존 프로젝트의 코어 후보 식별·분류 | 책임짐 | - |
| 새 프로젝트의 코어 제안·사용자 확정 | 식별 근거 제공 | `establishing-project-core` |
| 핵심 컨셉·뾰족한 재미·PoC 설계 | 코어 후보와 경계 제공 | `analyzing-and-refining-game-concepts` |
| 코어를 기획 책임 원본에 기록·발행 | 구조와 내용 제공 | `managing-design-documents` |
| 실제 변경의 계약·정적·런타임 검증 | 코어 회귀 기준 제공 | `reviewing-and-validating-project-changes` |
| 적대적 공격·비평 검증·개선 반복 | 코어 보호 기준 제공 | `running-adversarial-review-and-refinement` |
...
코드 코어는 기술 의존성의 중심일 수 있지만, 그 자체가 반드시 제품 정체성인 것은 아니다. 기획·시스템 코어와 기술 코어를 분리해 기록한다.

## Core versus MVP

- `CORE`: 프로젝트 정체성과 핵심 경험에 반드시 필요하다.
- `MVP_SUPPORT`: 정체성 자체는 아니지만 핵심 가설을 실행·관찰하기 위해 최소 제품에 필요하다.
- `BOTH`: 코어이면서 MVP에서도 반드시 구현해야 한다.
- `LATER`: 코어가 아니며 현재 MVP 검증에도 필요하지 않다.

예를 들어 강화 게임의 무기 강화는 `CORE`이자 `BOTH`일 수 있다. 저장 기능은 정체성의 코어가 아니어도 장기 진행을 검증하기 위한 `MVP_SUPPORT`일 수 있다.
...
6. 단지 UI·대사·스킨·콘텐츠 수량처럼 교체 가능한 외피인가?
...
- 현행 책임 원본과 최신 사용자 결정을 찾았다.
- 실제 코드·데이터·자산·테스트 상태를 확인할 수 있다.
- 핵심 행동·루프·주요 시스템 후보가 있다.
- 프로젝트 정체성과 기술 의존성을 구분할 입력이 있다.
- MVP 범위 또는 현재 검증 목표를 알고 있다.

## Definition of Done

- 기획·시스템·코드 코어를 분리했다.
- 각 코어 후보에 근거와 제거·대체 테스트가 있다.
...
- 선행·보완: `analyzing-and-refining-game-concepts`
- 사용자 확정: `establishing-project-core`
- 문서화·발행: `managing-design-documents`
