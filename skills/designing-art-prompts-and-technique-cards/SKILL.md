...
`ACTUAL_CONSUMER_REQUIRED`: 프로젝트 이미지 생성 전에 실제 게임/제품 소비처를 특정한다. 시스템 설명·세계관·관계도·제작 체크리스트처럼 제작자·AI가 알아야 할 `PRODUCTION_INFORMATION`은 `INFORMATION_ARTIFACT_NOT_IMAGE_ASSET`이며 `TEXT_TABLE_FLOW_DB_FIRST`로 route-out한다. 정보 자체는 계속 생성·갱신하되 설명용 생성 이미지 시트로 대체하지 않는다.
...
이미지 프롬프트를 만들기 전에 요청 목적을 먼저 분류한다.

```text
PRODUCTION_INFORMATION
→ 시스템 설명 / 세계관 / 캐릭터·세력 관계 / 관계도 / 제작 체크리스트 / 밸런스·경제 / Flow / 구현 계약
→ TEXT_TABLE_FLOW_DB_FIRST
→ Markdown / Notion text / table / DB / Mermaid / Flow / JSON 등 적합한 owner에 생성·갱신
→ image generation STOP
...
`PLAYER_FACING_EXPLANATORY`는 튜토리얼·도감·인게임 도움말·세력 관계 UI처럼 플레이어가 실제 게임에서 소비하는 설명 visual이다. 제작자·AI용 관계도나 시스템 문서와 구분한다.

consumer가 `DOCUMENTATION_DECORATION / AI_EXPLANATION_ONLY / CHECKLIST_DECORATION / UNNAMED_FUTURE_USE`뿐이면 `DO_NOT_GENERATE`다.

## Skill modes

- `technique-card`: 재사용 가능한 아트·UI 기술과 프롬프트 패턴을 `templates/planning/ART_TECHNIQUE_CARD.md`에 기록한다.
- `planning-visualization`: 실제 게임에 들어갈 구체적인 screen/scene/asset slot을 미리 검증하는 `PLANNED_GAME_SURFACE`만 이미지로 시각화한다. 제작자·AI용 세계관·관계·시스템 설명은 text/table/Flow/DB로 route-out한다.
- `intermediate-visual-checkpoint`: 사용자가 중간 점검·예상 게임 화면·UI 포함 화면을 요청하거나 P1 해석 위험이 있을 때, 현재 Project canon만으로 실제 planned game surface 한 화면 흐름을 `DRAFT_VISUAL` 또는 대체 와이어프레임으로 검토한다.
- `final-visual-candidate`: 승인된 기획을 바탕으로 Demo-First·게임 runtime·상점·홍보·UI·캐릭터 등 명시된 actual consumer에 사용할 고품질 후보를 만든다.
- `visual-qa-and-approval`: 생성물의 기획 일치성·actual consumer 적합성·실제 화면·구현 가능성·권리·오류·재사용성을 검수하고 승인 후보 상태를 판정한다.

## Core principle

좋은 아트 프롬프트는 형용사를 많이 나열하는 문장이 아니라 **실제 소비 목적, 유지할 정체성, 변경할 축, 화면 구성, 산출물 규격, 실패 기준**을 가진 제작 계약이다. **생성 결과는 자동 최종 자산이 아니다.**
...
- `ACTUAL_CONSUMER_REQUIRED`의 `consumer_kind`, `consumer_surface`, `primary_use`, `validation`.
...
사용자가 현재 대화에서 특정 프로젝트 이미지 한 장의 생성·편집을 명시적으로 요청했다면 그 요청을 현재 작업의 임시 requirement 입력으로 사용할 수 있다. 프로젝트 자산 작업에서는 실제 consumer를 함께 특정하며, 이 예외는 프로젝트 전체 자산 목록 선정·승인이나 `ASSET_MANIFEST.yml` 승격을 자동으로 만들지 않는다. 또한 해당 한 장과 무관한 coverage gap을 자동 생성 queue로 변환하지 않는다.

## Process

1. 먼저 요청이 `PRODUCTION_INFORMATION`인지 image asset/surface인지 판정한다. 시스템 설명·세계관·관계도·제작 체크리스트 등 정보 산출물이면 `TEXT_TABLE_FLOW_DB_FIRST`로 적합한 문서/표/DB/Mermaid/Flow owner에 생성·갱신하고 이미지 프롬프트 경로를 종료한다.
2. 이미지 작업이면 `ACTUAL_CONSUMER_REQUIRED`를 적용해 `consumer_kind / consumer_surface / primary_use / validation`을 특정한다. 유효 소비처가 없으면 `DO_NOT_GENERATE`다.
3. 프로젝트의 화면·캐릭터·시스템·asset set과 연결되는 작업이면 `docs/knowledge/game-development/GAME_VISUAL_ASSET_COVERAGE_CHECKLIST.md`에서 현재 consumer와 직접 관련된 coverage item만 확인한다. `COVERED_EXISTING / REQUIREMENT_LINKED / GAP_BLOCKING / GAP_NONBLOCKING / NOT_APPLICABLE`을 구분하고 **`NO_AUTOMATIC_IMAGE_GENERATION_FROM_GAPS`**를 적용한다.
4. 프로젝트 자산 작업이면 `Visual Requirement Gate` 선정 결과를 확인한다. `DEFER / CUT / REUSE_SYSTEM / REUSE_PROJECT`를 이미지 생성으로 임의 변환하지 않는다.
5. `PROJECT_RELATION_REQUIRED`를 확인한다. Project가 불명확하면 다른 프로젝트 자료를 빌리지 말고 `BLOCKED_UNVERIFIED`로 둔다.
6. `references/notion-project-visual-continuity-gate.md`에서 승인 reference와 `Keep / Avoid / Do Not Drift`를 확인한다. 승인 기준이 부족하면 `MISSING_CANON`, 상충하면 `VISUAL_CANONICAL_CONFLICT`다.
7. `planning-visualization`, `intermediate-visual-checkpoint`, `final-visual-candidate` 중 필요한 mode를 정한다.
8. 결과물이 쓰일 실제 screen/scene/asset slot 또는 product-distribution surface와 가장 먼저 전달할 정보를 정한다.
9. `STATE_FAMILY_COMPLETENESS`를 적용해 대표 한 장뿐 아니라 현재 consumer가 요구하는 Normal/Hover/Pressed/Disabled/Focus, Wind-up/Active/Recovery 같은 상태군을 확인한다. 불필요한 상태는 `NOT_APPLICABLE`로 둔다.
10. 원본에서 **유지할 요소 / 변경할 요소**를 분리하고, identity-preserving task라면 unchanged identity를 hard constraint로 둔다.
11. target resolution/aspect/crop과 필요한 engine consumption 조건을 정한다. pixel art, sprite sheet, UI 9-patch, mipmap/texture import 같은 조건은 실제 사용처에 필요할 때만 추가한다.
12. Pinterest를 포함한 발견 reference는 원작자·원출처·license·유사성을 확인하고 표면 복제를 금지한다.
13. 프롬프트를 다음 모듈로 작성한다.

```text
coverage_item_id · coverage_status · state_family_status
→ requirement_id · project_relation · 목적/역할
→ consumer_kind · consumer_surface · primary_use · validation
→ 관련 canon / Decision
...
14. 짧은 제어어가 필요하면 자연어 설명 뒤에 code/tag를 보조 어휘로 쓴다. FACS AU는 참고 어휘이지 모델 공식 명령 체계로 가정하지 않는다.
15. 포스터·UI mockup은 일러스트, 정보 슬롯, 실제 타이포그래피를 분리해 수정 가능하게 만든다. 설명용 정보 시트가 아니라 actual consumer가 있는 product/game surface인지 다시 확인한다.
16. 성공 사례뿐 아니라 실패 조건과 수정 프롬프트를 기록한다.
17. 생성 뒤 `visual-qa-and-approval`을 실행하고 `templates/planning/GPT_IMAGE_GENERATION_AND_REVIEW_PLAN.md`에 기록한다. actual consumer, coverage link와 state family가 실제 결과에서 유지됐는지도 확인한다.
18. 생성물을 correct Project의 candidate record/page에 붙이고 **readback**한다. upload success만으로 delivery success를 주장하지 않는다.
19. 승인된 Decision만 `PROJECT_ASSET_APPROVED`로 promotion하고 repository implementation task와 연결한다.
20. 실제 consumer에서 requirement/asset/runtime evidence가 확인되면 연결된 coverage row를 readback한다. coverage 자체를 runtime proof로 사용하지 않는다.
21. 모델·버전·입력 이미지·승인 reference·Project canon·target platform specification이 달라지면 필요한 범위를 재검증한다.

## Intermediate visual checkpoint

정본·Decision ID·실제 target screen/scene 제약이 없는 경우에는 이미지를 추정 생성하지 말고 `MISSING_CANON`으로 반환한다. 한 번에 한 실제 화면 흐름을 우선하고, 화면 목적·첫 시선·주요 행동·플랫폼/화면비/입력·위험/비용/보상·성공/실패/복구·긴 한글·접근성·확인 사실과 미결정을 Brief에 쓴다.

이미지 생성이 필요 없거나 적절하지 않으면 텍스트 와이어프레임·Mermaid·구조화 Screen record로 검토한다. 제작자·AI가 알아야 할 세계관·시스템·관계 정보는 이 비이미지 경로가 정상값이다.
...
3. 키 컬러와 재질·광원.
4. 이름·엠블럼·태그라인 영역.
5. 실제 consumer에 필요한 정보 모듈.
6. 표정·측면 inset이 실제 사용처에 필요할 때.
7. 타이틀·날짜·하단 정보가 product surface에 필요할 때.
8. 실제 후처리와 현지화 계획.
...
- production information이면 text/table/DB/Mermaid/Flow owner update — image generation 없이
- actual consumer가 있는 image generation/edit prompt
...
완료 보고에는 실제로 생성·업로드·readback·승인·runtime 검증한 항목과 실행하지 않은 항목을 분리한다. `PRODUCTION_INFORMATION`을 생성했다면 이미지가 없어도 정상 산출물로 기록한다.