...
## 1. Authority before location

`backup`, `[백업]`, `archive`, `old`라는 경로명은 보존 위치만 설명한다. 다음 조건을 모두 만족해야 현재 권한 제거가 성립한다.

1. 현재 정본이 별도 경로에 명시돼 있다.
2. 활성 Documentation Map·Registry·START_HERE가 archive를 현행 원본으로 읽지 않는다.
3. 구현 계획과 자동 Router가 archive 항목을 선택하지 않는다.
...
| `EVIDENCE_RETENTION` | 승인·테스트·결함·감사 증거 | 주장·버전·실행 상태 보존 |
...
- 원문을 그대로 보존한다. 현재 관점에 맞게 과거 문장을 다시 쓰지 않는다.
- metadata header를 추가할 수 있지만 본문을 비우지 않는다.
- 활성 문서의 link와 backtick path는 현재 정본으로 바꾼다.
- archive 문서를 기본 콜드 스타트 목록, current canon registry 또는 자동 실행 계획에 두지 않는다.
...
## 7. Generated derivatives

PDF, DOCX, diagram과 export는 다음을 가진다.

```text
source path
source commit or input hash
generator and version
freshness status
publication manifest link
```

stale derivative는 archive하거나 재생성하며 현재 발행본으로 표시하지 않는다.

## 8. Code and runtime assets

활성 source tree에 도달 불가능 코드와 미사용 runtime asset을 장기 보존하지 않는다. 고유 구현은 Git history, tag, release 또는 별도 archival repository로 복구 가능하게 한 뒤 승인된 migration에서 active path에서 제거한다. 대형 binary는 저장 비용·license·redistribution 계약을 별도로 검토한다.

## 9. Secrets

다음은 archive 대상이 아니다.

- API token과 access token
- password와 credential
- private key
- session cookie
- 개인식별정보 또는 규제 데이터

발견 즉시 revoke·rotate·remove 절차를 적용한다. 사고 기록에는 secret 원문을 복제하지 않는다.

## 10. Branch and tag retention
...
삭제 기능이 없으면 closed PR과 검증된 tag를 보존 증거로 사용하고 `branch deletion: NOT_RUN`으로 기록한다. 장기 `archive/*` branch는 활성 branch 탐색을 오염시키므로 기본 보존 방식으로 사용하지 않는다.

## 11. Manifest verification

각 레코드는 고유 `current_path`와 `archive_id`를 사용한다. `superseded_by`는 저장소에 존재하는 경로 또는 `external:` prefix를 가진다. Markdown 본문은 metadata를 제외하고도 비어 있지 않아야 한다. hash와 rollback ref는 실제 원문에 연결돼야 한다.
