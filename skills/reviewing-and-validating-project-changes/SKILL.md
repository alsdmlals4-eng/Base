...
8. 검증 실행 모드는 정확히 두 개다. 공개 저장소에서 standard GitHub-hosted runner가 현재 검증 대상의 `ci-gate` Check Run을 만들 수 있으면 `REMOTE_CI`가 기본이다. 비용 0은 `LOCAL_FALLBACK` 선택 조건이 아니다.
9. `ci-gate` Check Run이 실패·취소·queued·in progress를 포함해 하나라도 존재하면 원격 검증 소유권을 유지한다. 테스트·workflow 실패를 local status로 대체하지 않는다.
10. Actions 인프라·권한·서비스 문제로 현재 PR head와 current test merge SHA에 `ci-gate` Check Run 자체가 생성되지 않은 경우에만 다음을 실행할 수 있다.
...
11. `LOCAL_FALLBACK`은 exact local/PR head, clean worktree, 최신 `origin/<base>` ancestry, head/test-merge의 `ci-gate` Check Run 부재를 검증 전후 확인하고 기존 `tools/run_local_validation.py`가 성공한 경우에만 exact head SHA에 `context=ci-gate`, `state=success` commit status를 발행한다.
12. 검증 중 Actions가 복구되어 `ci-gate` Check Run이 생기거나 SHA/worktree/base 조건이 변하면 fail-closed하고 status를 발행하지 않는다.
13. fallback precondition 또는 필요한 로컬·플랫폼·runtime 증거를 재현할 수 없으면 다음 상태를 유지한다.
...
14. blocked 보고에는 workflow, event 또는 dispatch 입력, 현재 mode, head/base SHA, 기대 Required Check, 확인할 log·artifact, 재개 조건, 미검증 위험을 남긴다.
15. Actions 사용 가능 상태가 복구되면 최신 branch·SHA·workflow diff를 다시 확인하고 `REMOTE_CI`의 아직 필요한 검증을 실행한다.
...
- `LOCAL_FALLBACK`을 사용했다면 exact head, clean worktree, current base ancestry, head/test-merge의 `ci-gate` Check Run 부재를 검증 전후 확인했다.
...
- `ci-gate` Check Run이 존재하는 실패·취소·대기 상태를 `LOCAL_FALLBACK` status로 덮는다.
- 검증한 SHA와 다른 commit에 fallback success status를 발행한다.
- Actions가 사용 불가한데 workflow 존재만으로 통과를 주장하거나 fallback 조건 검증 없이 성공 처리한다.
