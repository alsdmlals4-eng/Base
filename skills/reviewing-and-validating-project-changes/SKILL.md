...
description: Use when code, data, documents, assets, configuration, or external AI output must be checked against an approved work contract and verified with repository evidence, reference freshness, accessibility barriers, performance budgets, runtime behavior, and regression evidence before integration.
---
...
변경 주체가 사람이든 AI든 설명보다 실제 diff, 책임 원본, 실행 결과를 우선한다. 승인된 작업 계약과 관찰 가능한 증거가 일치하고, 변경된 정본이 모든 활성 소비자에 전파되며, 적용되는 접근성·성능 기준을 충족하기 전에는 완료로 판정하지 않는다.
...
5. 접근성·성능·플랫폼 기준이 적용되는 변경인지 판정한다.
...
7. 접근성·성능 개선이 핵심 규칙·가독성·아트·입력 약속을 훼손하지 않는지 확인한다.
