...
현재 프로젝트 권위 모델은 `DOMAIN_SPLIT_CANON`이다.

```text
NOTION_HUMAN_FACING_CANON
→ 사람이 읽고 비교·학습·수정하는 Project Home, Visual/Story Bible, Flow/Storyboard, 예산표·Tier표·로스터·경제·성장표

REPOSITORY_STRUCTURED_CANON
→ Markdown·JSON·game data·code·scene·resource·test·runtime evidence
```

승인된 기획 결정은 대화나 checkpoint 대기열에만 남기지 않는다. 결정 직후 GitHub 추적 근거와 `CURRENT_CONFIRMED_DECISIONS.md`, 영향받는 구조화 책임 원본을 갱신하고, 사람이 실제로 이해·수정해야 하는 내용은 같은 승인 단위에서 프로젝트 Notion 사람용 surface에 투영한 뒤 readback한다. Notion 수정이 Markdown·JSON·game data·code·Scene·Resource·Test 또는 runtime 의미 변경을 요구하면 `SYNC_BEFORE_IMPLEMENTATION`으로 repository에 먼저 동기화한다.

공용 승인 동기화 계약은 `docs/CONFIRMED_DECISION_SYNC_POLICY.md`, workspace 권위는 `docs/operations/PROJECT_WORKSPACE_AUTHORITY_CONTRACT.json`, 사람용 시각·표 계약은 `docs/VISUAL_COLLABORATION_TOOL_POLICY.md`를 따른다.
...
사람용 전체 그림·시각·예산·Tier·비교표 → NOTION_HUMAN_FACING_CANON
구조화 기획·데이터·구현 계약 → REPOSITORY_STRUCTURED_CANON
실제 구현·runtime 상태 → REPOSITORY_RUNTIME_TRUTH
사람 기본 발행 열람 → Registry 정책이 요구하는 PDF
Word 검토 → 선언한 경우의 선택 DOCX
최신성 → Publication Manifest + repository SHA + Notion last sync/readback
...
`CURRENT_CONFIRMED_DECISIONS.md`는 현재 승인 Decision의 핵심·대체 관계·상세 정본 경로·Commit·Notion 반영 위치를 책임지고, 시스템 상세 규칙은 등록된 분야 원본이 책임진다.

## Project Notion human-facing responsibility
...
Notion에서 사람용 의미를 수정하면 먼저 `PROPOSED_NOTION_CHANGE`로 해석한다. 단순 배치·표현 편집은 Notion에서 끝날 수 있다. 하지만 수정이 structured spec/data/runtime 동작을 바꾸면 Decision ID와 영향 경로를 확인하고 repository를 `SYNC_BEFORE_IMPLEMENTATION`으로 동기화한 뒤 `SYNCED`로 올린다.

예산표·Tier표에는 가능하면 `CONFIRMED / PROVISIONAL / DEFERRED / REJECTED`, Decision ID 또는 canonical path, source main SHA/freshness locator, Notion last sync를 함께 남긴다.
...
- unique material은 올바른 repository/Notion 책임면으로 이관하고 destination readback한다.
...
DOCX와 다이어그램은 Registry가 선언한 경우만 생성한다. `CURRENT`, Notion 사람용 승인, runtime 검증, 사람 시각 검수 완료는 서로 독립 상태다.

승인 결정의 즉시 정본화와 PDF·DOCX 발행 주기는 별개다. `milestone_sync` 문서라도 승인 Decision은 즉시 구조화 원본과 필요한 Notion 사람용 surface에 반영하고, 파생 발행본은 정책이 정한 시점에 생성한다.
...
4. 프로젝트 Notion Project Home과 관련 사람용 surface가 있으면 현재 표·시각·Decision locator·last sync를 읽는다.
5. Google Sheets는 `COMPATIBILITY_ONLY`이고 unique migration material이 실제로 남은 경우에만 추가로 확인한다.
6. 문서 ID·책임 범위·포함·제외와 **Notion 우선 도메인인지 repository 우선 도메인인지** 확정한다.
...
- 사람이 비교·학습해야 하는 예산·Tier·로스터·경제·성장 정보는 Notion 표를 우선 고려한다.
- Notion 표는 machine-consumed JSON/game data의 별도 복제 구현이 아니다.
...
→ 필요한 Notion Project Home / Visual / Flow / 사람용 확정표 갱신
→ Notion destination readback
→ Decision·Commit·Notion 표현·대체 관계 대조
→ SYNCED / PROPOSED_NOTION_CHANGE / SYNC_FAILED / BLOCKED_UNVERIFIED 판정
```
...
- repository 구조화 원본만 갱신했는데 필요한 Notion 사람용 표현이 오래됐거나, 반대로 Notion만 의미 변경했는데 repository 반영이 필요한 상태를 `SYNCED`로 보고하지 않는다.
- 실제로 수행하지 않은 검수·플레이테스트·CI·Notion readback은 `UNVERIFIED`를 유지한다.
...
- Notion 사람용 표·Flow·Visual이 해당 도메인의 현재 Decision과 일치
- 누락·충돌·중복·대체 누락 0건
- GitHub `main`과 필요한 Notion 표현 일치
- 미결정·보류·기각 항목 보존
- 관련 구현·테스트·발행 상태 구분
...
- 필요한 Notion 사람용 surface readback과 current Decision 일치
- Notion 의미 변경이 repository structured/runtime 변경을 요구할 경우 `SYNC_BEFORE_IMPLEMENTATION` 충족
...
같은 작업에서 Registry, 관련 책임 원본, `CURRENT_CONFIRMED_DECISIONS.md`, 필요한 Notion 사람용 surface, Roadmap, Project Skill, Active Context, Documentation Map과 발행 상태를 맞춘다.

하위 시스템 checkpoint에서는 누적 GitHub Decision ID와 책임 원본 반영 위치, Notion 사람용 표현을 대조하고 미반영·충돌·중복·대체 누락을 0으로 만든다. 미결정 항목은 삭제하지 않고 명시적으로 보류한다.

PR 또는 직접 `main` Decision Commit 뒤에는 `running-adversarial-review-and-refinement`로 최근 승인 누락·정본 충돌·Notion drift·회귀를 검사한다.
...
- 실행 mode:
- 문서 ID·책임 범위:
- authority domain: NOTION_HUMAN_FACING_CANON | REPOSITORY_STRUCTURED_CANON | BOTH
- 책임 원본·형식·경로:
- CURRENT_CONFIRMED_DECISIONS 반영:
- Notion Project Home·사람용 surface:
- Notion readback·last sync:
- 발행 정책:
- 실제 변경:
- 승인 Decision 추적 surface·ID:
- 기존 Decision·PR·Notion 비교:
- main Commit SHA:
...
- 질문 전에 기존 Decision·정본·열린 PR·최근 병합 PR·Notion을 대조했다.
- 승인 Decision이 GitHub 추적 근거에 기록됐다.
- 승인 Decision이 즉시 `CURRENT_CONFIRMED_DECISIONS.md`와 분야 책임 원본에 반영됐다.
- 승인 구조화 문서가 `main`에 반영되고 Commit SHA가 기록됐다.
- 필요한 Notion 사람용 surface를 갱신하고 destination readback 결과가 일치한다.
- Notion 변경이 structured/runtime 변경을 요구하면 구현 전 repository 동기화를 완료했다.
...
- 새 작업자가 repository 구조화 책임 원본과 Notion 사람용 출력·다음 작업을 모두 찾는다.
...
- 사람이 보는 예산·Tier·Flow의 의미가 바뀌었는데 Notion을 갱신하지 않음
...
- 승인 Decision은 기존 방식대로 `CURRENT_CONFIRMED_DECISIONS.md`와 분야 structured owner에 즉시 반영하고, 사람이 봐야 하는 변화는 필요한 Notion surface에 동기화한다.
