...
description: Use when installing a persistent game-project repository operating system that connects schema v3 Markdown or JSON sources, current PDFs, optional derivatives, selective skills, images, tests, gates, GitHub governance, and handoff across new chats.
---
...
- 활성 `[기획서]`가 저장소 루트에 있다.
- 프로젝트 전체와 선택한 분야별 본책은 Registry에 등록된 단일 Markdown 또는 JSON 책임 원본이다.
...
운영 라우터는 Markdown을 유지하고 프로젝트·분야 기획 본책은 문서 성격에 따라 Markdown 또는 JSON 단일 책임 원본을 선택한다.

## Phase 3 — Install project and discipline design bibles

프로젝트 전체와 프로젝트가 실제로 선택한 책임 분야를 `DESIGN_DOCUMENT_REGISTRY.json`에 등록한다. 11개 분야는 공용 카탈로그일 뿐 모든 프로젝트에 강제하지 않는다.
...
작은 프로젝트는 여러 책임을 한 Markdown 또는 JSON 원본에 통합할 수 있지만 `responsibility_coverage`에서 누락 없이 선언한다.
