...
description: Use when installing a persistent game-project repository operating system that connects JSON design bibles, human DOCX/PDF publications, selective skills, images, tests, gates, GitHub governance, and handoff across new chats.
---
...
- 활성 `[기획서]`가 저장소 루트에 있다.
- 프로젝트 전체와 분야별 본책은 구조화 JSON이다.
- `DESIGN_DOCUMENT_REGISTRY.json`이 모든 책임과 경로를 연결한다.
- 각 활성 기획서에 DOCX·PDF·자동 다이어그램·승인 이미지 자산·Manifest가 있다.
- `SKILL_REGISTRY.json`과 사람용 `PROJECT_SKILL_MAP.docx/.pdf/.assets`가 있다.
...
- PR 검사로 JSON·DOCX·PDF·이미지·스킬 갱신 누락을 발견한다.
...
8. 프로젝트·분야 본책은 JSON, 사람 열람본은 DOCX/PDF다.
...
운영 라우터는 Markdown을 유지할 수 있지만 프로젝트·분야 기획 본책은 JSON으로 만든다.

## Phase 3 — Install project and discipline design bibles

프로젝트 전체와 11개 책임 분야를 `DESIGN_DOCUMENT_REGISTRY.json`에 등록한다.
...
분야_기획서.docx
분야_기획서.pdf
분야_기획서.assets/generated/
분야_기획서.assets/approved/
분야_기획서_PUBLICATION_MANIFEST.json
```

작은 프로젝트는 여러 책임을 한 JSON에 통합할 수 있지만 `responsibility_coverage`에서 누락 없이 선언한다.
...
## Phase 5 — Install selective project skill system
...
기획서 JSON의 `approved_visuals`에서 사람용 DOCX/PDF에 포함할 자료를 지정한다.
...
- 세 Governance Checker가 정상·실패 시나리오에서 검증됐다.
